...
- `_bmad-output/planning-artifacts/epics.md` - UPDATE (amended 2026-05-14 via Scope-Override). Synchronize the Epic List summary with the existing detailed body (Epics 13/14/15) and normalize heading levels. Driven by Sprint Change Proposal 2026-05-13.
- `_bmad-output/planning-artifacts/sprint-change-proposal-2026-05-13.md` - CREATE (amended 2026-05-14 via Scope-Override). Authorizes the `epics.md` summary update.
...
- Scope deviation (resolved 2026-05-14): the Hexalith.EventStore submodule pointer was advanced from `7ec370e` to `1e52117` in the implementation commit. See the Scope-Override section below for the authorizing rationale; this corrects the prior assertion that "no submodule state was touched."
...
- 2026-05-14: 3-layer adversarial code review (Blind + Edge + Auditor); 3 decisions resolved + 8 patches applied + 9 items deferred (15.2-RV1..RV9). File Scope amended for `epics.md` and the sprint-change-proposal; Scope-Override block added below for the Hexalith.EventStore submodule pointer bump.

### Scope-Override (added 2026-05-14)

Three deviations from the original File Scope were identified by the Acceptance Auditor and are formally accepted here:

1. **Hexalith.EventStore submodule pointer bump `7ec370e` → `1e52117`.** Forbidden-by-default per File Scope lines on submodule changes and per Dev Notes guardrail "Do not change root-level submodule pointers." Accepted because the working tree had already advanced the pointer in alignment with independent Hexalith.EventStore Epic 22 work (24 commits including authorization-failure reason codes, projection adapter contract, query handling, and several story drafts). None of those EventStore commits is required by Story 15.2's registry/validation work; they are an unrelated submodule sync that was bundled into this commit. Re-open trigger: any reviewer asks why an unrelated submodule pointer landed here; future story authors should land submodule bumps in their own commit instead.
2. **`_bmad-output/planning-artifacts/epics.md` edited.** Originally Read/verify only. Accepted because the Epic List summary was stale (stopped at Epic 12 while the detailed body and `sprint-status.yaml` already tracked Epics 13/14/15), and the companion sprint-change-proposal documents the synchronization as planning-doc maintenance. Heading-level normalization (`### Epic N` → `## Epic N` in the detailed body) and the unrelated Story 1.1 title rename were bundled in; both should ideally have been their own commits, recorded here for traceability.
3. **`_bmad-output/planning-artifacts/sprint-change-proposal-2026-05-13.md` created.** Originally outside Allowed/Possible/Read-only lists. Accepted because the proposal is the authorizing artifact for change #2 and follows the standard sprint-change-proposal pattern.

Carries-forward: future commits should keep submodule pointer bumps in dedicated commits, and planning-artifact edits in dedicated commits, to keep File Scope contracts predictable.
...
### Decisions needed (resolved 2026-05-14)

- [x] [Review][Decision] **D1 - Hexalith.EventStore submodule pointer bump** — Resolved: keep + add Scope-Override block. See the new "Scope-Override (added 2026-05-14)" section in the Dev Agent Record. Completion Notes corrected.
- [x] [Review][Decision] **D2 - `epics.md` + `sprint-change-proposal-2026-05-13.md` outside File Scope** — Resolved: amend File Scope retroactively + Scope-Override block. Both files now listed under Allowed files with the amendment note. Story 1.1 title rename remains bundled (recorded in Scope-Override).
- [x] [Review][Decision] **D3 - Auth-mode validation moved AFTER rate-limit/dimension** — Resolved: accept new order and pin with tests. Added `remarks` XML doc to `Validate(...)` documenting the 17-step ordering contract; added `Validate_OrderingContract_AuthModeFiresAfterRateLimitWhenBothInvalid` and `Validate_OrderingContract_ProviderRegistryFiresBeforeModelShapeRegex` to pin the contract. Converted to patch P8.

### Patches (resolved 2026-05-14)

- [x] [Review][Patch] **P1 - Registry → `ImmutableArray`** [`src/Hexalith.Memories.Server/Ingestion/EmbeddingProviderDefaults.cs:37-76,329-345`] — Done: `Registry` is `ImmutableArray<ProviderRegistryEntry>`; `ProviderRegistryEntry.Models` is `ImmutableArray<ModelRegistryEntry>`; `ModelRegistryEntry.AllowedDimensions` is `ImmutableArray<int>`. Added `using System.Collections.Immutable;`.
- [x] [Review][Patch] **P2 - Misleading "DimensionsAtSharedMaximum" test name** — Done: renamed to `Validate_UnknownModelAtSharedDimensionMaximum_ShouldThrowUnknownModelError`, added a comment explaining why the unknown-model error wins at the inclusive boundary, and asserted that the dimension cap fragment ("16384") is NOT in the message.
- [x] [Review][Patch] **P3 - Tautological mixed-case test** — Done: theory now passes canonical provider/model as separate args, asserts the input deliberately differs from canonical casing (`provider.ShouldNotBe(canonicalProvider)`), then Validate accepts AND the config preserves the caller's casing.
- [x] [Review][Patch] **P4 - No-op `with` clause on Ollama rate-limit test** — Done: `RateLimitPerMinute` switched from `6000` (Ollama default — no-op) to `30_000` (well above Google ceiling 3_000, well below Ollama ceiling 60_000). Added a comment explaining the intent.
- [x] [Review][Patch] **P5 - `isOllama` re-derived from `config.Provider`** — Done: now `provider.ProviderName == OllamaProviderName` using the canonical registry value.
- [x] [Review][Patch] **P6 - Unused `ModelRegistryEntry.DefaultDimensions`** — Done: field removed. Defaults remain in the `CreateDefaultConfig` lambdas; the registry no longer carries dead state.
- [x] [Review][Patch] **P7 - Missing non-leakage assertions on error messages** — Done: added `Validate_UnknownProviderError_ShouldNotLeakModelNames` (unsupported-provider message lists providers only) and `Validate_GoogleConfigWithOllamaModel_ShouldNotLeakOllamaModelNameInError` (wrong-provider×Ollama-model error does not quote OllamaModelName).
- [x] [Review][Patch] **P8 - Pin new auth-mode-after-rate-limit ordering** (converted from D3) — Done: added `remarks` XML doc on `Validate(...)` describing the 17-step ordering contract; added `Validate_OrderingContract_AuthModeFiresAfterRateLimitWhenBothInvalid` and `Validate_OrderingContract_ProviderRegistryFiresBeforeModelShapeRegex` to pin the contract.
...
- 2026-05-14 patch round: 8 patches applied (P1..P8). Patches added 5 new test methods to `EmbeddingProviderDefaultsTests` and 0 to other test files. Patch validation owed by the Dev Agent: rerun `dotnet test tests/Hexalith.Memories.Server.Tests/Hexalith.Memories.Server.Tests.csproj --filter "FullyQualifiedName~EmbeddingProviderDefaultsTests"` and `dotnet build Hexalith.Memories.slnx`. The review applied edits but did not execute them.
