...
SPEC_ACTIVATION_CONTRACT = """1. Resolve customization: `uv run {project-root}/_bmad/scripts/resolve_customization.py --skill {skill-root} --project-root {project-root} --key workflow`. On failure, read `{skill-root}/customize.toml` directly.
...
DEPRECATED_GENERATE_FORWARD_PATTERN = re.compile(
    r"(?is)invoke [`']bmad-project-context[`'] with .*?\bsetup\b.*? intent"
)
