...
3-layer adversarial code review (Blind Hunter + Edge Case Hunter + Acceptance Auditor) on 2026-05-14 against commit `57819b4`. Raw findings: 33 → 26 unique after dedup → 3 decision-needed + 7 patch + 9 defer + 7 dismissed.

### Acceptance Criteria audit summary

- **AC1 Pass** — Single registry (`Registry` array in `EmbeddingProviderDefaults`) replaces scattered checks; `Google()`/`Ollama()` delegate to `FindRequiredProvider().CreateDefaultConfig()`.
- **AC2 Pass** — `MaxSupportedDimensions = 16_384` rejects out-of-policy dimensions; `Validate_DimensionsAboveSharedMaximum_ShouldThrowAtConfigTime` pins behavior.
- **AC3 Pass** — Closed registry rejects cross-pollination by construction; three named negative tests cover Google×qwen3, Ollama×gemini, and syntactically-valid-but-unregistered models.
- **AC4 Pass** — Case-insensitive validation with caller-casing preserved; `OrdinalIgnoreCase` lookups in `FindProvider`/`FindModel`; `EmbeddingClient.ParseEmbeddingProviderIdentifier` lowercases provider and preserves model verbatim (verified at `src/Hexalith.Memories.Server/Ingestion/EmbeddingClient.cs:148-155`, unchanged).
- **AC5 Partial** — Broad named coverage present; gap noted as W1 below (contract-tier serialization test not updated; acceptable per Task 2 accept-disposition but worth recording).
- **AC6 Pass** — All invalid combinations throw `ArgumentException` before `TenantConfigurationActor.SetStateAsync`, before any index path can consume the config.

### Decisions needed

- [ ] [Review][Decision] **D1 - Hexalith.EventStore submodule pointer bump in this commit** [Hexalith.EventStore@7ec370e->1e52117] — The diff bumps the EventStore submodule pointer, but spec lines 118-119 forbid `Hexalith.EventStore/**` and "Any submodule pointer change", spec line 195 reiterates "Do not change root-level submodule pointers", and the story's own Completion Notes line "No submodule state was touched" directly contradicts the diff. Per stored memory `feedback_submodule_init.md`, this is a known sensitive rule. **Choose:** (a) revert the pointer in a follow-up commit and correct the Completion Notes truth-claim; (b) keep the pointer change but add a formal Scope-Override block to the Dev Agent Record citing the EventStore commits being incorporated and why this story is the right vehicle; (c) move the pointer change to its own commit on `main` with no story association.
- [ ] [Review][Decision] **D2 - `epics.md` edited + new `sprint-change-proposal-2026-05-13.md` outside File Scope** [`_bmad-output/planning-artifacts/epics.md`, `_bmad-output/planning-artifacts/sprint-change-proposal-2026-05-13.md`] — Spec line 94 lists `epics.md` as Read/verify only; the diff modifies it (added Epic 13/14/15 summary, demoted detailed-section heading levels `###` → `##`, renamed Story 1.1 title). The companion sprint-change-proposal file is not in any of the Allowed / Possible / Read-only lists. **Choose:** (a) split planning-artifact updates into a separate commit/story (e.g. the sprint-change-proposal itself); (b) add a Scope-Override block citing the proposal as the authorizing artifact and amend the story's File Scope to include them; (c) revert the planning-artifact changes from this commit.
- [ ] [Review][Decision] **D3 - Auth-mode validation moved AFTER rate-limit/dimension checks** [`src/Hexalith.Memories.Server/Ingestion/EmbeddingProviderDefaults.cs:181-198`] — Before this commit, an invalid `AuthMode` was the second error surfaced (after provider/model). The new ordering puts AuthMode validation after Dimensions, MaxSupportedDimensions, model allowlist, AllowedDimensions, RateLimit>0, and RateLimit-ceiling checks. A config that is invalid on AuthMode plus any other field now reports a different error than before; no test pins the order. **Choose:** (a) restore AuthMode to its previous position (before Dimensions); (b) accept the new order and document it as the design intent in XML doc + a new ordering test that pins the contract; (c) defer to a follow-up `15.2-RVx` if the order is judged immaterial to consumers.

### Patches (unambiguous fixes)

- [ ] [Review][Patch] **P1 - Registry exposed as mutable `ProviderRegistryEntry[]`** [`src/Hexalith.Memories.Server/Ingestion/EmbeddingProviderDefaults.cs:37-76,328`] — `readonly` protects the reference but not element replacement; closed-allowlist guarantee weakens. Switch to `ImmutableArray<ProviderRegistryEntry>` (and `ImmutableArray<int>` for `AllowedDimensions`, `ImmutableArray<ModelRegistryEntry>` for `Models`).
- [ ] [Review][Patch] **P2 - `Validate_DimensionsAtSharedMaximumForUnsupportedModel_ShouldThrowBeforeIndexUse` name is misleading** [`tests/Hexalith.Memories.Server.Tests/Ingestion/EmbeddingProviderDefaultsTests.cs`] — `Dimensions = 16_384` (= `MaxSupportedDimensions`) passes the upper-bound gate and fails on the unknown-model check ("future-model"). The test's name implies max-dim coverage but the assertion is unknown-model. Rename to `Validate_UnknownModelWithDimensionsAtSharedMaximum_ShouldThrowUnknownModelError` and assert the unknown-model error fragment specifically. Optionally add a true boundary test (e.g. registered model with `AllowedDimensions = [16_384]` if/when registry grows).
- [ ] [Review][Patch] **P3 - `Validate_MixedCaseProviderAndModel_ShouldUseCaseInsensitiveRegistryLookup` is tautological** [`tests/Hexalith.Memories.Server.Tests/Ingestion/EmbeddingProviderDefaultsTests.cs`] — Asserts `config.Provider.ShouldBe(provider)` after Validate, but records are immutable so this is trivially true. Strengthen to prove case preservation through validation: assert that the validated input differs from canonical casing (e.g. `provider.ShouldNotBe(EmbeddingProviderDefaults.GoogleProviderName)` for input `"Google"`), and Validate accepts; then optionally round-trip through `EmbeddingClient.ParseEmbeddingProviderIdentifier($"{provider}:{model}")` to verify provider is canonicalized and model preserved.
- [ ] [Review][Patch] **P4 - `Validate_OllamaRateLimitAboveGoogleCeilingButBelowOllamaCeiling_ShouldNotFallBackToGoogle` has a no-op `with` clause** [`tests/Hexalith.Memories.Server.Tests/Ingestion/EmbeddingProviderDefaultsTests.cs`] — `EmbeddingProviderDefaults.Ollama() with { RateLimitPerMinute = 6000 }` — `6000` is already the Ollama default, so the `with` does nothing. The test name claims to test a value above Google ceiling 3000 and below Ollama ceiling 60000, but the actual exercised value (6000) happens to satisfy that, but it's a default coincidence. Change to e.g. `RateLimitPerMinute = 30_000` to genuinely exercise the Ollama-only boundary.
- [ ] [Review][Patch] **P5 - `isOllama` re-derived from `config.Provider` after registry lookup** [`src/Hexalith.Memories.Server/Ingestion/EmbeddingProviderDefaults.cs:200`] — `bool isOllama = string.Equals(config.Provider, OllamaProviderName, StringComparison.OrdinalIgnoreCase);` — but `provider.ProviderName` is already the canonical registry value at this point. Use `provider.ProviderName == OllamaProviderName` (or add a typed `IsOllama` helper on `ProviderRegistryEntry`). Eliminates two parallel sources of provider truth and removes a divergence risk if the registry ever introduces aliases.
- [ ] [Review][Patch] **P6 - `ModelRegistryEntry.DefaultDimensions` is unused** [`src/Hexalith.Memories.Server/Ingestion/EmbeddingProviderDefaults.cs:343`] — Set to `768` (Google) and `2560` (Ollama) but never read; defaults are hardcoded in the `CreateDefaultConfig` lambdas. Either remove the field, or wire `CreateDefaultConfig` to read `model.DefaultDimensions` so the registry stays self-consistent. Recommend removal — adding a use creates a tighter contract that should be its own decision.
- [ ] [Review][Patch] **P7 - Missing negative test: wrong-provider unknown-model message should NOT leak the other provider's model list** [`tests/Hexalith.Memories.Server.Tests/Ingestion/EmbeddingProviderDefaultsTests.cs`] — `Validate_UnknownModelForProvider_ShouldThrowAndListProviderModels` covers wrong-provider×unknown-model but only with `"totally-fake"` model name. Add an assertion that an unsupported-provider error message lists *only* registry providers (no leaked model names), and that an unsupported-model-for-Google error message does *not* list `OllamaModelName` (already partially covered via `ShouldNotContain`, but only for `"totally-fake"`; extend to combinations where the model name belongs to another provider).

### Deferred (pre-existing, recorded for follow-up)

- [x] [Review][Defer] **W1 - AC5: `TenantEmbeddingConfigSerializationTests` not updated** — Task 2 chose `accepted` for casing (no canonicalization at contract boundary), so contract-tier serialization is unchanged by design; spec wording leaves room for this. Recording for traceability.
- [x] [Review][Defer] **W2 - Actor reindex tests lost same-Provider/different-Model isolation** [`tests/Hexalith.Memories.Server.Tests/Actors/TenantConfigurationActorTests.cs:95-135`] — Tests changed from `Google() with { Model = "text-embedding-004" }` (provider-stays, model-changes) to `Ollama()` (provider-changes, model-changes). Same-provider/model-only reindex-trigger coverage is now lost. Hard to restore inside this story because the registry currently lists only one model per provider; expand-and-fix becomes possible when Google or Ollama gains a second registered model.
- [x] [Review][Defer] **W3 - Other test files still use unregistered model literals** [`tests/Hexalith.Memories.Server.Tests/Search/HybridSearchServiceTests.cs:74`, `tests/Hexalith.Memories.Server.Tests/Endpoints/TenantEmbeddingConfigEndpointTests.cs:30,39`] — Use `"text-embedding-004"` / `"different-model"` literals that compile only because these tests do not call `Validate`. Pre-existing; will silently fail if those code paths later add validation preflight. Out of File Scope for this story.
- [x] [Review][Defer] **W4 - `EmbeddingClient.IsGoogle/IsOllama` dispatch is hardcoded** [`src/Hexalith.Memories.Server/Ingestion/EmbeddingClient.cs:150`] — Future registry providers will fail `ParseEmbeddingProviderIdentifier` and embedding-generation dispatch because the parser/generator binary-checks `IsGoogle || IsOllama` rather than consulting the registry. Closed-allowlist behavior of `EmbeddingProviderDefaults` does not extend to `EmbeddingClient`. Architectural follow-up, not in this story's File Scope.
- [x] [Review][Defer] **W5 - `GenerateEmbeddingActivity` may emit mixed-case provider in persisted identifier** [`src/Hexalith.Memories.Server/Activities/Ingestion/GenerateEmbeddingActivity.cs`] — Tenant-persisted `Provider = "Google"` (mixed case, now accepted by Validate) would flow into the identifier string `"Google:gemini-embedding-001"`. `ParseEmbeddingProviderIdentifier` lowercases the provider on read, so persisted-write and parsed-read forms diverge for equality comparisons. Out of File Scope. Related to W4 — both want a registry-driven canonicalization at the persisted identifier boundary.
- [x] [Review][Defer] **W6 - Migration tool uses binary Google/Ollama coin-flip, not registry** [`src/Hexalith.Memories.Server/Migration/EmbeddingVectorMigrationService.cs:125-149`] — `TargetProvider` defaults fall back to `Ollama()` if not Google. Related to W4. Out of File Scope.
- [x] [Review][Defer] **W7 - Whitespace-prefixed provider not trimmed before registry lookup** [`src/Hexalith.Memories.Server/Ingestion/EmbeddingProviderDefaults.cs:136-145`] — `ArgumentException.ThrowIfNullOrWhiteSpace` passes for `" google"`, then `FindProvider` misses (no trim). Operator sees "Provider ' google' is not supported" without hinting at whitespace. Already deferred as `13.1-RV4` family.
- [x] [Review][Defer] **W8 - Already-persisted invalid configs not surfaced on read** [`src/Hexalith.Memories.Server/Actors/TenantConfigurationActor.cs:86-98`] — Closed-registry validation runs on write only; tenants whose state was valid under loose rules continue to be served without warning. Story 15.2 documents this as intentional (compatibility behavior). Worth tracking for operator-visibility design in a follow-up story.
- [x] [Review][Defer] **W9 - "Order-sensitive metric test passed in isolation" mentioned in Dev Agent Record** [`_bmad-output/implementation-artifacts/15-2-provider-model-dimension-registry.md:254`] — Test ordering dependency acknowledged but not fixed; not caused by this story.

### Dismissed (false positives or non-issues)

- **DR1 - "Validate(null) should throw NullReferenceException, not ArgumentNullException"** (Blind Hunter #2) — False positive. `ArgumentNullException.ThrowIfNull(config);` at `EmbeddingProviderDefaults.cs:135` exists; diff context window cut it off.
- **DR2 - "Dimensions 16384 or less" message misleading because no registered model accepts 16384** (Blind Hunter #5) — Message is technically correct; the upper-bound policy is the contract, not the registry's current capacity. Operators who hit this error are by definition outside the registry.
- **DR3 - "ParseEmbeddingProvider mixed-case behavior unverified vs deferred-work claim"** (Blind Hunter #11) — Verified: `EmbeddingClient.cs:148-155` lowercases provider via `ToLowerInvariant()` and preserves model verbatim, exactly as the `13.3-RV8` accepted-disposition states.
- **DR4 - "FindRequiredProvider runtime lookup risk"** (Blind Hunter #12) — Registry is a static field initialized at type load; the only failure mode is a programmer deleting the entry, which a unit test would catch.
- **DR5 - "FormatNumbers ungrouped/unquoted"** (Blind Hunter #6) — Quoting integers in error messages is incorrect; existing `FormatQuoted` is for names. The asymmetry is intentional and correct.
- **DR6 - "int.MinValue dimensions test missing"** (Edge Case Hunter #14) — `Dimensions <= 0` at line 154 handles all non-positive values including `int.MinValue`; an explicit `int.MinValue` test would be redundant.
- **DR7 - "Test count 141/141 PASS unauditable"** (Blind Hunter #13) — Acceptance Auditor verified the counts are plausible against the diff's test additions; the concern about order-sensitive metric test is captured as W9.

### Validation

- Tests claimed by Dev Agent Record: `EmbeddingProviderDefaultsTests` 141/141, `EmbeddingClientTests` 64/64, `TenantConfigurationActorTests` 28/28, `TenantEmbeddingConfigSerializationTests` 6/6, full `Hexalith.Memories.Server.Tests` 1763/1763, `dotnet build Hexalith.Memories.slnx` 0W/0E. Acceptance Auditor verified counts as plausible from the diff; not re-run by review.
- 3-layer adversarial review evidence at `_bmad-output/implementation-artifacts/15-2-provider-model-dimension-registry.md#review-findings`.
