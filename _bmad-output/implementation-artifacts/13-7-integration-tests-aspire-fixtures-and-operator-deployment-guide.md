...
_Code review on 2026-05-03 by adversarial parallel layers (Blind Hunter, Edge Case Hunter, Acceptance Auditor) against commit `30c6910`._

- [x] [Review][Patch] Fix `fake!` null race in `OllamaOidcFakeServer.StartAsync` [`tests/.../Fixtures/OllamaOidcFakeServer.cs:80-86`] — `ScriptedHttpServer.StartAsync` returns a listening server before `fake = new ...` runs; an inbound request between those statements dereferences null. Construct the instance first, then start the inner server.
- [x] [Review][Patch] Snapshot `secrets.json` content before assigning `_secretsFilePath` [`tests/.../Fixtures/AspireIngestionPipelineFixture.cs:400-404`] — if `File.ReadAllText` throws after `_secretsFilePath` is set, `RestoreLocalDaprSecret` deletes the user's pre-existing secrets file (data loss).
- [x] [Review][Patch] Handle non-object JSON root in `secrets.json` [`tests/.../Fixtures/AspireIngestionPipelineFixture.cs:407-410`] — `JsonNode.Parse(content)?.AsObject()` throws `InvalidOperationException` for arrays/scalars, not null. Use `as JsonObject ?? throw` with a clear error.
- [x] [Review][Patch] Validate `EmbeddingProviderSecret.Name` against an identifier regex [`tests/.../Fixtures/EmbeddingProviderSecret.cs:11`] — current YAML interpolation in `CreateDaprConfigOverrideIfNeeded` is safe only because the single caller passes `memories-embedding-client-secret`. Enforce `^[A-Za-z0-9._-]+$` in the record so future callers cannot inject YAML.
- [x] [Review][Patch] Restore typed JSON serialization in `ProvisionActiveTenantAsync` [`tests/.../Fixtures/AspireIngestionPipelineFixture.cs:295-303`] — change dropped `MemoriesJsonContext.Options` and switched to anonymous object, breaking AOT/source-gen consistency with the rest of the file. Extend the typed `TenantProvisioningInput` with optional `vectorDimensions` and pass `MemoriesJsonContext.Options`.
- [x] [Review][Patch] Replace bearer-redaction substring assertions with token-value assertions [`tests/.../Fixtures/OllamaOidcFakeServerTests.cs:79`, `tests/.../Ingestion/OllamaEmbeddingEndToEndTests.cs:120`] — `evidence.ShouldNotContain("Bearer ")` is a coincidence (record `ToString()` never produces it); `logs.ShouldNotContain("Bearer " + "eyJ")` cannot detect leak of the opaque `example-bearer-token`. Assert against the actual token value (already done for the secret).
- [x] [Review][Patch] Annotate operator-doc "Ollama local/no-auth" column as config-schema only [`docs/operations/embedding-providers.md:36-48`] — the matrix presents it as a first-class option but row `AuthMode` admits "not accepted by the current Ollama runtime dispatch". Add an explicit "(not yet runtime-accepted)" header annotation so operators do not configure a path the server rejects.
- [x] [Review][Patch] `ResolveRepositoryRoot` must throw when `Hexalith.Memories.slnx` is not found [`tests/.../Fixtures/AspireIngestionPipelineFixture.cs:489-501`] — currently returns `Directory.GetCurrentDirectory()` silently; tests then write/restore a `secrets.json` in an arbitrary location.
- [x] [Review][Patch] Wrap `JsonDocument.ParseAsync` in `HandleEmbedAsync` with try/catch returning 400 [`tests/.../Fixtures/OllamaOidcFakeServer.cs:184`] — non-JSON body throws and the fake returns 500 instead of asserting the contract violation.
- [x] [Review][Patch] Validate `JsonValueKind` before `GetString()` in `HandleEmbedAsync` [`tests/.../Fixtures/OllamaOidcFakeServer.cs:191-193`] — `model`/`input` as null/number throws `InvalidOperationException`; reject with 400 instead.
- [x] [Review][Patch] Dispose `_fakeServer` if `_fixture.InitializeAsync` throws [`tests/.../Ingestion/OllamaEmbeddingEndToEndTests.cs:30-37`] — xUnit does not call `DisposeAsync` after `InitializeAsync` failure; the Kestrel listener leaks until process exit.
- [x] [Review][Patch] Distinguish workflow-failed from wait-expired in `WaitForSemanticHashAsync` timeout [`tests/.../Ingestion/OllamaEmbeddingEndToEndTests.cs:214-230`] — current code breaks the loop on terminal `Failed` status then throws a generic `TimeoutException`, masking the real failure cause.
- [x] [Review][Patch] Guard `WorkflowReachedTerminalStatus` JSON parse [`tests/.../Ingestion/OllamaEmbeddingEndToEndTests.cs:233-260`] — non-JSON or empty body aborts the wait loop with an unhandled `JsonException` instead of polling.
- [x] [Review][Patch] Scrub client secret/token from `WaitForSemanticHashAsync` timeout message [`tests/.../Ingestion/OllamaEmbeddingEndToEndTests.cs:227-230`] — `lastWorkflowPayload` and `logs` are embedded raw in the exception; if the workflow ever surfaces request payload or response headers, secrets leak into test output.
- [x] [Review][Patch] Reject duplicate form values in `HandleTokenAsync` [`tests/.../Fixtures/OllamaOidcFakeServer.cs:152-155`] — duplicated `grant_type`/`client_id` produce ambiguous "malformed" errors; assert `StringValues.Count == 1` per required field with a distinct error.
- [x] [Review][Defer] End-to-end uses Redis `KEYS` in 3-minute polling loop [`tests/.../Ingestion/OllamaEmbeddingEndToEndTests.cs:192`] — deferred, test infrastructure choice; bounded test data and `DisableParallelization` make `KEYS` acceptable. Auditor accepted AC2.
- [x] [Review][Defer] URL-escape `tenantId`/`canary` in search query [`tests/.../Ingestion/OllamaEmbeddingEndToEndTests.cs:107`] — deferred, defensive only; both are Guid hex without URL-reserved characters.
- [x] [Review][Defer] Clean up parent temp directory in `DeleteTempDaprConfig` [`tests/.../Fixtures/AspireIngestionPipelineFixture.cs:474-487`] — deferred, low impact; AppHost-generated component yamls leak per random `daprAppId`.
- [x] [Review][Defer] Consolidate duplicate `ResolveRepositoryRoot` helpers (AppHost + fixture) — deferred, would require shared test/AppHost helper crossing project boundaries.
- [x] [Review][Defer] Truncate or rewrite `sprint-status.yaml` history comment lines — deferred, project-wide pattern; would require coordinated convention change.
- [x] [Review][Defer] Add dedicated `[Fact]` cases for each AC4 malformed-token rejection branch (missing `Content-Type`, missing `grant_type`, missing `client_id`, missing `client_secret`, malformed body) — deferred, fake rejects them at runtime; coverage gap rather than behavior gap.
- [x] [Review][Defer] Replace `EmbedRequestCount.ShouldBeGreaterThanOrEqualTo(2)` magic number with a named constant or rationale comment [`tests/.../Ingestion/OllamaEmbeddingEndToEndTests.cs:116`] — deferred, low-priority test improvement.
...
| Date       | Change                                                                                                                                                                                                                                   | Author |
|------------|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|--------|
| 2026-05-03 | 3-layer adversarial code review (Blind Hunter + Edge Case Hunter + Acceptance Auditor) closed 15 patches + 7 deferred (13.7-RV1..RV7) + 5 dismissed. Patches: fake-server null-race fix in `StartAsync`, secrets.json snapshot ordering + non-object root guard, `EmbeddingProviderSecret` regex validation (YAML injection), restored typed `TenantProvisioningInput` JSON serialization, bearer/secret redaction asserts actual token values (not coincidental substrings), operator doc Ollama local/no-auth annotated as config-schema-only, `ResolveRepositoryRoot` fails closed when slnx is absent, `HandleEmbedAsync` JSON-parse + ValueKind guards, `_fakeServer` disposed on fixture init failure, workflow-failed distinguished from wait-expired in timeout, `WorkflowReachedTerminalStatus` returns enum + JSON-parse guard, secret/token scrubbed from timeout exception payload/logs, `HandleTokenAsync` rejects duplicate form values. Validation: full solution build 0W/0E, Story13_7 4/4 Pass, OllamaOidcFakeServerTests 3/3 Pass, default Google/fake `IngestionPipelineTests.PostIngest_ShouldIndexMemoryUnitAcrossAllBackends` 1/1 Pass. Story moved `review` -> `done`. | Claude Opus 4.7 |
