<!-- Review cadence: update when an `app.MapX("/api/v1/…")` route is added/removed/renamed, when the pub/sub delivery or subscription-discovery route changes, when a health-probe path changes, or when the MCP transport route changes; otherwise quarterly — whichever comes first. Last reviewed: 2026-07-04. -->
...
- **Service invocation — the `/api/v1/*` table.** A caller reaches a REST route through Dapr service invocation at `/v1.0/invoke/memories/method/<path>`. The ACL `operation` is the route path **without the leading slash** (the `method/…` segment), the `app` is `memories`, and `httpVerb` is the row's HTTP method. Translate each table row directly: `operation` = `method` joined with the path column, `httpVerb` = the method column. For example, the search row maps to operation `method/api/v1/search` with verb `GET`, and the primary ingest row to operation `method/api/v1/ingest` with verb `POST`.
...
All 56 routes below are minimal-API endpoints mapped in `src/Hexalith.Memories.Server/Program.cs` (`app.MapGet/Post/Put/Delete/Patch`). There are no `MapGroup`/route-group prefixes; each path is the full template. The drift-guard test derives this list from the source, so a newly added endpoint that is not documented here fails the build.

| Area | Method + path | Purpose |
| :--- | :--- | :--- |
| Ingestion | `POST /api/v1/ingest` | Start a content-ingestion workflow. |
| Ingestion | `GET /api/v1/ingest/{instanceId}` | Read a tenant-authorized safe ingestion workflow status projection (`IngestionWorkflowStatus`), not raw Dapr `WorkflowState`. |
| Ingestion | `POST /api/v1/ingest/url` | Ingest content from a URL. |
| Ingestion | `POST /api/v1/ingest/directory` | Ingest a directory batch. |
| Ingestion | `GET /api/v1/ingest/batches/{batchId}` | Read directory-batch ingestion status after authorizing the stored batch tenant before per-file status fan-out. |
| Tenants | `GET /api/v1/tenants/{tenantId}/embedding-config` | Read the tenant embedding configuration. |
| Tenants | `PUT /api/v1/tenants/{tenantId}/embedding-config` | Replace the tenant embedding configuration. |
| Tenants | `POST /api/v1/tenants` | Provision a tenant. |
| Tenants | `GET /api/v1/tenants/{tenantId}/provision-status/{instanceId}` | Read tenant-provisioning workflow status. |
| Tenants | `GET /api/v1/tenants` | List tenants. |
| Tenants | `GET /api/v1/tenants/{tenantId}` | Read a tenant. |
| Tenants | `GET /api/v1/tenants/{tenantId}/configuration` | Read the resolved tenant configuration. |
| Tenants | `PATCH /api/v1/tenants/{tenantId}` | Update the tenant display name. |
| Tenants | `DELETE /api/v1/tenants/{tenantId}` | Start tenant deletion. |
| Tenants | `GET /api/v1/tenants/{tenantId}/deletion-status/{instanceId}` | Read tenant-deletion workflow status. |
| Tenants | `POST /api/v1/tenants/{tenantId}/verify` | Verify a tenant. |
| Consistency | `POST /api/v1/tenants/{tenantId}/consistency/verify` | Start a consistency-verification run. |
| Consistency | `GET /api/v1/tenants/{tenantId}/consistency/verify/{instanceId}` | Read consistency-verification status. |
| Consistency | `GET /api/v1/tenants/{tenantId}/consistency/inspect/{memoryUnitId}` | Inspect a memory unit's consistency. |
| Consistency | `POST /api/v1/tenants/{tenantId}/consistency/repair` | Start a consistency-repair run. |
...
| Cases | `GET /api/v1/tenants/{tenantId}/cases/{caseId}/status` | Read case status. |
| Cases | `GET /api/v1/tenants/{tenantId}/cases/{caseId}/failed-units` | List a case's failed memory units. |
| Memory units | `GET /api/v1/tenants/{tenantId}/cases/{caseId}/memory-units/{memoryUnitId}` | Read a memory unit. |
| Memory units | `GET /api/v1/tenants/{tenantId}/cases/{caseId}/memory-units/by-source-uri` | Resolve a source URI to its canonical memory-unit id by exact key (Story 18.5; literal segment, beats the `{memoryUnitId}` template). |
| Memory units | `POST /api/v1/tenants/{tenantId}/cases/{caseId}/memory-units/{memoryUnitId}/re-ingest` | Re-ingest a single memory unit. |
| Memory units | `POST /api/v1/tenants/{tenantId}/cases/{caseId}/failed-units/re-ingest` | Re-ingest all failed units in a case. |
| Cases | `GET /api/v1/tenants/{tenantId}/cases/{caseId}/activity` | Read case activity. |
| Members | `PUT /api/v1/tenants/{tenantId}/cases/{caseId}/members/{memberId}` | Add or update a case member. |
| Members | `DELETE /api/v1/tenants/{tenantId}/cases/{caseId}/members/{memberId}` | Remove a case member. |
| Members | `GET /api/v1/tenants/{tenantId}/cases/{caseId}/members` | List case members. |
| Memory units | `DELETE /api/v1/tenants/{tenantId}/cases/{caseId}/memory-units/{memoryUnitId}` | Delete a memory unit. |
| Cases | `DELETE /api/v1/tenants/{tenantId}/cases/{caseId}` | Delete a case. |
| Annotations | `POST /api/v1/tenants/{tenantId}/cases/{caseId}/memory-units/{memoryUnitId}/annotations` | Add an annotation to a memory unit. |
| Annotations | `GET /api/v1/tenants/{tenantId}/cases/{caseId}/memory-units/{memoryUnitId}/annotations` | List a memory unit's annotations. |
| Search | `GET /api/v1/search` | Hybrid (semantic + keyword + graph) search. |
| Graph | `GET /api/v1/tenants/{tenantId}/traverse` | Traverse the tenant relation graph. |
| Telemetry | `GET /api/v1/tenants/{tenantId}/telemetry/summary` | Read the tenant telemetry summary. |
| Handlers | `GET /api/v1/handlers` | Inspect the registered-handler snapshot. **Experimental (`HXL002`).** |
...
| Derived stores | `PUT /api/v1/tenants/{tenantId}/diagnostics/derived-stores/{storeClass}/{resourceId}` | Put a tenant-scoped metadata-only diagnostic derived-store entry. |
| Derived stores | `GET /api/v1/tenants/{tenantId}/diagnostics/derived-stores/{storeClass}/{resourceId}` | Read a tenant-scoped metadata-only diagnostic derived-store entry. |
| Derived stores | `GET /api/v1/tenants/{tenantId}/diagnostics/derived-stores/{storeClass}` | List tenant-scoped metadata-only diagnostic derived-store entries. |
| Derived stores | `DELETE /api/v1/tenants/{tenantId}/diagnostics/derived-stores/{storeClass}/{resourceId}` | Delete a tenant-scoped metadata-only diagnostic derived-store entry. |
| Derived stores | `POST /api/v1/tenants/{tenantId}/derived-stores/bindings` | Finalize an ingestion-created derived-store binding. |
| Derived stores | `POST /api/v1/tenants/{tenantId}/derived-stores/corrections` | Start or rejoin a derived-store correction workflow. |
| Derived stores | `GET /api/v1/tenants/{tenantId}/derived-stores/corrections/{operationId}` | Read derived-store correction status. |
...
- **Count tie:** the number of extracted `/api/v1/*` source routes (currently **56**) must equal the REST table's data-row count — defending against silent omission, duplicate rows, and phantom rows.
...
- `src/Hexalith.Memories.Server/Program.cs` plus `Endpoints/*Endpoints.cs` — the 56 `/api/v1/*` `app.MapX` route registrations; `MapDefaultEndpoints()`; middleware order `UseCloudEvents()` → `MapControllers()` → `MapSubscribeHandler()`.
