...
Status: done
External Action Pending: AC #6 — branch protection on `main` is pending maintainer-only configuration per [`docs/dev/branch-protection.md`](../../docs/dev/branch-protection.md). Story is code/doc-complete; AC #6 enforcement is blocked on GitHub repository settings until applied by a repository admin after the first CI workflow run on `main` exposes the `build`, `test-unit-contract`, and `integration-fast` check names. The Tasks/Subtasks under AC #6 (3.4, 3.5, 4.4) are deliberately marked `[x]` because they document the required settings and activation sequence; the in-GitHub apply step lives outside the repository.
...
- [x] [Review][Defer→Closed] **W10: AC #6 marked `[x]` though `docs/dev/branch-protection.md` says "remains pending maintainer action"** — bookkeeping only; the spec text already captures the dependency, but the task checkbox is misleading. Source: auditor. Story file AC #6. **Closed by course correction (2026-04-26):** added `External Action Pending` status line at the top of this story file calling out AC #6's maintainer-only GitHub-settings dependency; W10 moved to `Closed by: course correction (2026-04-26)` in `deferred-work.md`.
...
- 2026-04-26 (course correction — W10 closure): P1 resolved by commit `5eecf4c` (Jerome) which committed `package-lock.json` together with the rest of the bundled 11.1 + 11.2 work. Added an `External Action Pending` status line at the top of this story file to make AC #6's pending-maintainer dependency unmissable; the AC text already captured the dependency but the `[x]` task checkboxes alone were misleading (W10). W10 moved from `Deferred` to `Closed by: course correction (2026-04-26)` in `deferred-work.md`.
