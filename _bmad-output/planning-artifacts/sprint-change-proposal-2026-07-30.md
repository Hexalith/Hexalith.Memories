...
> **ANCHOR CORRECTION 2026-08-01 by code review (second pass).** Every line anchor written into this file by the 2026-07-31 correction - including the second-approval note appended to this very document - is stale in the file it cites, because the same edits that were verified pre-write then moved the lines. Re-derived against this file at HEAD: the rewritten `deferred-work.md` authorization row cited as `:118` is at **`:122`**; the `epics.md` authorization row cited as `:115` is at **`:119`**; `:305` is at **`:309`**; "Approval authorizes **only** the artifact edits enumerated here" cited as `:351` is at **`:359`**; success criteria 4 and 5 cited as `:365-368` are at **`:371-375`**; the "complete proposal" approval wording cited as `:404` / `:410-411` is at **`:423` / `:429`**; and Annex A cited as `:413-464` ("approximately 55 lines") spans **`:434-492`, 59 lines**. No substantive claim changes - every one of them still holds at its re-derived location. This is the failure mode of `## L14`, added to `story-creation-lessons.md` in the same commit, whose closing rule is "Fail closed: do not write an inherited anchor into `epics.md`, a story file, `sprint-status.yaml`, or a proposal without re-deriving it first."
