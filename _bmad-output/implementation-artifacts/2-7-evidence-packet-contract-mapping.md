...
### Review Findings — 2026-06-30 (re-review at review gate)

Code-review run on 2026-06-30 at the `review` gate (3 adversarial layers: Blind Hunter, Edge Case Hunter, Acceptance Auditor; Opus 4.8). Diff scope: story baseline `ca5c2e0` → `HEAD`, restricted to the story File List (history is bundled across mislabeled commits, so no clean range exists). Whitespace/CRLF noise stripped. The 2026-05-20 review's deferred items were cross-referenced; only **new** findings (or carry-over fixes that missed a call site) are raised below.

#### Decision-needed

- [x] [Review][Decision→Patch] **Evidence strength applies normalized [0,1] thresholds to raw/unnormalized axis scores** — **RESOLVED 2026-06-30 (Jerome): emit `EvidencePacketEvidenceStrength.Unknown` for non-normalized axes (syntactic single-axis, graph); keep thresholds for normalized/composite. Now a patch.** — `DetermineEvidenceStrength` (`EvidencePacketMapper.cs:234-263`) classifies `<0.4 weak / <0.75 moderate / >=0.75 strong`, but `ScoredResult.Score` is documented "raw BM25 for syntactic axis" (`ScoredResult.cs:16`) and `--axis syntactic` is a valid CLI/MCP axis (`SearchQueryCommand.cs`). Single-axis syntactic (and graph) results feed raw, unbounded scores into the normalized thresholds, so strength is systematically fabricated (BM25 commonly >=0.75 → always `strong`). Distinct from the already-deferred score=0→None+Complete item. Options: (a) emit `EvidencePacketEvidenceStrength.Unknown` (the existing, currently-unused enum value) for non-normalized axes; (b) restrict strength to the normalized composite/semantic axes; (c) normalize per-axis (couples mapper to scoring internals — violates the purity constraint). Recommended: (a)/(b). Severity: Major. AC1.
- [x] [Review][Decision→Patch] **CLI error envelope serializes server `message`/`suggestion` verbatim alongside the sanitized packet** — **RESOLVED 2026-06-30 (Jerome): sanitize/bound server message+suggestion at the CLI envelope and strengthen the forbidden test to assert the whole envelope is clean. Now a patch.** — `CliErrorWriter` writes `CliErrorPayload(code, message, suggestion)` (`CliErrorPayload.cs:17`, `CliErrorWriter.cs:45`) into the same JSON envelope as the carefully-sanitized `evidencePacket`. Only the packet's recovery guidance passes `SanitizeGuidance`; the raw `error.message`/`error.suggestion` do not. The `SearchQuery_HybridJson_ServerForbidden_*` test asserts only `packet` cleanliness and never inspects `root.error`, giving false assurance. The leak is pre-existing CLI behaviour (server-responsibility), but the packet's sanitization promise makes the asymmetry notable. Decide: sanitize/bound server error text at the CLI envelope, or treat it as trusted-upstream and document it — either way, strengthen the test to assert the whole envelope is clean. Severity: Major. AC2 / sanitization hardening.

#### Patch

- [x] [Review][Patch] **`IsUnauthorized` misses HTTP_401/HTTP_403 (and ACCESS_DENIED-style codes) → 401/403 mapped to `degraded`+`retry`** [src/Hexalith.Memories.Contracts/V1/EvidencePacketMapper.cs:526-528] — `FromError` (`:148`) calls `IsUnauthorized`, which only matches the substrings `UNAUTHORIZED`/`FORBIDDEN`. `ErrorResponseDecoder.cs:46` synthesizes `Code = "HTTP_" + statusCode` for non-JSON/malformed auth responses, and the CLI `ErrorMessageCatalog` has explicit `HTTP_401`/`HTTP_403` entries — so a genuine 403/401 yields `state: degraded`, `omittedDetails.reason: backendUnavailable`, `recovery: retry`. Reachable via the CR10 CLI `search query` error path. Violates the load-bearing "unauthorized must dominate" invariant. Severity: High. Fix: recognize `HTTP_401`/`HTTP_403` and common auth codes (`ACCESS_DENIED`, `PERMISSION_DENIED`) as unauthorized.
- [x] [Review][Patch] **`SensitiveTextRegex` under-matches connection schemes / most paths and over-matches benign English** [src/Hexalith.Memories.Contracts/V1/EvidencePacketMapper.cs:540] — under-match: only redis/falkor + drive-letter + `/home/`/`/users/` are caught; `postgres://`/`mongodb://`/`bolt://`/`neo4j://`, ADO/JDBC connection strings (`Server=...;Password=...`), UNC `\\host\share`, and `/var//etc//opt//tmp//root/` paths pass `SanitizeGuidance` into `recovery[].guidance` verbatim. Over-match: `\bat\s+\w+\.` matches ordinary prose ("Arrived at noon.", "Look at logs.") and silently swaps useful guidance for the generic fallback. Severity: Major. Fix: broaden scheme/path coverage; replace the English-prose `at ...` pattern with an anchored token (e.g. `\bat\s+\w+(\.\w+)+` for stack-frame-style only).
- [x] [Review][Patch] **Hybrid `MapOmittedReason` fed `result.Degraded` instead of `effectiveDegraded`** [src/Hexalith.Memories.Contracts/V1/EvidencePacketMapper.cs:101 vs 102] — the M3 carry-over fix introduced `effectiveDegraded = result.Degraded || result.AllEnabledAxesUnavailable == true` for `DetermineState`/`Evidence.Degraded`, but `MapOmittedReason(result.OmittedReason, result.Degraded)` still uses the raw flag. When `AllEnabledAxesUnavailable==true` & `Degraded==false`: state=`degraded` yet `omittedDetails.reason` resolves to `none`/`tokenBudget` instead of `backendUnavailable`/`combined`, and the `backendDiagnostics` detail group is dropped — the two notions of degraded disagree within one packet. Severity: Minor. Fix: pass `effectiveDegraded`.
- [x] [Review][Patch] **`FromSearchResult`/`FromHybridSearchResult` do not zero `Sources`/`AxisEvidence` for an Unauthorized scope (latent)** [src/Hexalith.Memories.Contracts/V1/EvidencePacketMapper.cs:26-74, 86-135] — `sources` and `BuildAxisEvidence` are computed before `DetermineState`. If a caller passes `scope.IsolationStatus==Unauthorized`, state becomes `Unauthorized` and `BuildOmittedDetails` reports `fieldNames: ["sources","evidence"]` (withheld) while the top-level `sources`/`axisEvidence` remain fully populated from the rows — self-contradictory, and a row-level exposure path. `FromError` correctly emits empty sources; these two do not. Currently unreachable (CLI/MCP success paths hardcode `Authorized`), but a defensive gap in new code. Severity: Minor (latent). Fix: short-circuit to empty sources/evidence when the scope is unauthorized.
- [x] [Review][Patch] **Empty (non-null) `AxesUsed = []` suppresses row-derivable axis evidence** [src/Hexalith.Memories.Contracts/V1/EvidencePacketMapper.cs:38, 98] — the row-derived fallback (`result.AxesUsed ?? ...`) fires only on `null`. A result carrying `AxesUsed = []` with scored rows and no `Explanation` yields empty `axesUsed`/`axisEvidence` even though per-axis best scores exist — null-vs-empty asymmetry. Severity: Low. Fix: fall back when null-or-empty (confirm `[]` is not an intentional "no axes" signal).
- [x] [Review][Dismissed] **CLI `evidencePacketFactory` can build `EvidencePacketScope` with a null `TenantId`** — **DISMISSED 2026-06-30: not reachable; `tenantId` is validated (IsNullOrWhiteSpace early-return, SearchQueryCommand.cs:131-138) before the error-packet factory is installed.** [src/Hexalith.Memories.Cli/Commands/SearchQueryCommand.cs:239-242] — the error-packet factory closes over `tenantId!` and calls `CreateEvidenceScope(tenantId!, caseId)`; if a server error fires while `tenantId` is unresolved, `"tenantId": null` is emitted for a non-nullable contract field. Severity: Low. Fix: guard/validate `tenantId` before installing the factory, or emit empty string. Verify whether `--tenant` is guaranteed non-null at this point (if so, dismiss).
- [x] [Review][Patch] **Docs example out of sync with the contract** [docs/dev/cli-output-formats.md] — the `evidencePacket` example shows only `scope`/`state`/`evidence`, orders `state` before `evidence`, and uses a caveat string ("Confidence scores measure…") that differs from `DefaultCaveat` ("Scores measure query-result relevance, …"). But `result`/`sources`/`graph`/`omittedDetails`/`recovery` have no `JsonIgnore` and always serialize (the serialization test asserts `"sources":[]`). Severity: Nit. Fix: correct the example fields/order and caveat text.

#### Defer (new)

- [x] [Review][Defer] **`HasIndexedMemoryUnits` captured but never consulted — true-empty not distinguished from query-empty** [src/Hexalith.Memories.Contracts/V1/EvidencePacketMapper.cs:61] — `TotalCount==0` with `HasIndexedMemoryUnits==false` (nothing ingested) and `==true` (indexed, no match) both yield `state: empty` + `broadenScope`; for an unindexed tenant "broaden scope" is misleading. Deferred: enhancement, needs a recovery/state design choice.
- [x] [Review][Defer] **`state: empty` emitted when `returnedCount==0` but `totalCount>0` and nothing omitted** [src/Hexalith.Memories.Contracts/V1/EvidencePacketMapper.cs:224] — the `returnedCount==0` short-circuit ignores `totalCount`, producing a self-contradictory empty packet. Deferred: depends on upstream producing a contradictory result; low likelihood.
- [x] [Review][Defer] **Graph summary hardcoded `Available=false`/empty even when hybrid emits a `graph` axisEvidence entry with a real `GraphScore`** [src/Hexalith.Memories.Contracts/V1/EvidencePacketMapper.cs:71, 132] — internal inconsistency (graph axis evidence present, graph summary unavailable). Tolerable per spec (explicit-unavailable); graph mapping is optional/out of scope here. Deferred.
- [x] [Review][Defer] **`EvidencePacketSource` has no freshness field** [src/Hexalith.Memories.Contracts/V1/EvidencePacket.cs:63-72] — architecture grammar lists `sources: … freshness`; not an AC1-enumerated field. Deferred: grammar-alignment enhancement.

#### Re-confirmed (already deferred 2026-05-20 — no new action)

- Score=0/negative → `evidenceStrength: none` while `state: complete` (deferred line ~351); Combined budget+degraded `recovery[]` lacks `IncreaseTokenBudget` (line ~359); `permissionsContext` divergence across surfaces (line ~353); `McpErrorPayload` not in source-gen — confirmed harmless today via `DefaultJsonTypeInfoResolver` fallback, AOT-only (line ~354); `EvidencePacket` member on lower-level `SearchResult`/`HybridSearchResult` (line ~357); `MapOmittedReason` future-enum-values swallowed — confirmed `OmittedReason` enum exposes none today (line ~349); MCP known-tenant errors use `UnknownScope()` (line ~348).

#### Dismissed as noise (baseline-diff artifacts — not introduced by story 2.7)

- `MemoryUnitIdLookupResponse` / `["search lookup"]` registry / `SearchIndexEntryChanged|Removed` / `Dictionary<string,string>` registrations — introduced by commit `535b96b` (story 18.5), captured only because the review baseline spans bundled main-line history.
- MCP `service` value `memories-server` → `memories` (and Keycloak doc paragraph) — introduced by commit `ca8b54a` (repo-wide rename) / security wiring, not story 2.7.

#### Patch application log (2026-06-30)

All 9 patch findings applied (the 2 decision items resolved by Jerome became patches); F11 dismissed as unreachable. Verified against a clean build (`TreatWarningsAsErrors`) and green focused suites.

- **F1** applied — `IsUnauthorized` now recognizes `HTTP_401`/`HTTP_403`/`ACCESS_DENIED`/`PERMISSION_DENIED`.
- **F2** applied (decision: emit Unknown) — `AxesProduceNormalizedScores` gates strength; syntactic/graph single-axis now report `Unknown` instead of a fabricated grade.
- **F3** applied — `SensitiveTextRegex` broadened (db connection schemes, `/var` `/etc` `/opt` `/tmp` `/root` paths, UNC, `password=`) and the prose `at <word>.` over-match tightened to dotted stack-frame names only.
- **F4** applied — new `BuildUnauthorizedPacket` short-circuit zeroes sources/evidence for an unauthorized scope in both single-axis and hybrid mappers.
- **F5** applied — hybrid `MapOmittedReason` now fed `effectiveDegraded`.
- **F9** applied — `AxesUsed` falls back to row-derived axes on null-or-empty.
- **F15** applied — `docs/dev/cli-output-formats.md` example completed and reordered with the always-present fields.
- **F16** applied (decision: sanitize at CLI envelope) — server `message`/`suggestion` fallback routed through the shared `EvidencePacketMapper.SanitizeFreeText`; the forbidden test now asserts the whole error envelope is clean, not just the packet.
- **F11** dismissed — not reachable; `tenantId` is validated before the error-packet factory is installed.

Test evidence (DiffEngine_Disabled, dotnet exec on xUnit v3 dlls): Contracts.Tests 549/549, Cli.Tests 415/415, Mcp.Tests 83/83, Server.Tests EvidencePacket subset 8/8 — 0 failures. Build: 0 warnings, 0 errors.
