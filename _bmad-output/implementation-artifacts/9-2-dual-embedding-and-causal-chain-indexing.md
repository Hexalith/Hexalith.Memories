...
- 2026-04-25 — **Session 6 (committed-branch review D2/D6/D7 closure).** Closed the three remaining "still to implement" decision items from the 2026-04-24 committed-branch adversarial review. Detail:
    - **D2 — WorkflowReplaySafetyHostedService startup probe + `_metadata.Name` drill-through.** Verified via reflection probe that `Dapr.Workflow.WorkflowState` does NOT expose `Name` as a public property — it wraps the data in a private `_metadata` field of type `Dapr.Workflow.Client.WorkflowMetadata` (which IS public and exposes `Name`). The previous `GetProperty("Name", Public | Instance)` reflection therefore always returned null, `ShouldCountWorkflow` never matched, and the gate silently passed with count=0 — disabling the quiesce entirely. Fix: cached `FieldInfo MetadataField` + drill `state._metadata.Name`. Added `probedReflectionOnce` guard in `TryCountInFlightAsync` that fails Critical `9173` with reason `workflow-name-reflection-null` on the FIRST observed in-flight instance if reflection returns null — surfacing SDK drift as an operator alert rather than a silent no-op. Consolidated the existing `9173` sidecar-unreachable path into a single `LogGateFailedOpen(reason, exception)` helper (single EventId per LoggerMessage class is a source-gen requirement). Guard test `WorkflowStateMetadataField_StillExists_OnCurrentSdkSurface` asserts the private field + type + `WorkflowMetadata.Name` public property are present — a future SDK rename now fails fast at unit-test time. 5/5 `WorkflowReplaySafetyHostedServiceTests` green.
    - **D6 — StringComparer.Ordinal pinned on IngestionInput + IndexInput metadata.** Both contracts now construct `new Dictionary<string, MetadataField>(StringComparer.Ordinal)` as the auto-backed default; the `init` accessor rebuilds the assigned dictionary with Ordinal comparer even if the caller passed a looser one (e.g., `OrdinalIgnoreCase`), so the contract boundary normalizes the semantic. `BuildIndexMetadata` in `IngestionWorkflow` updated to use the `new Dictionary<string, MetadataField>(IDictionary<…>, IEqualityComparer<…>)` constructor — the one-arg copy overload does NOT carry the source comparer (common footgun). Audit of construction sites: `CloudEventToIngestionInputMapper` already uses Ordinal; `MemoriesClient.IngestAsync` + `QuickstartSampleFlow` already use Ordinal. No further call-site edits needed. 4/4 `IngestionInputMetadataComparerTests` green.
    - **D7 — ConsistencyVerificationResult.Notes split.** Re-verification confirmed Session 5's `ConsistencyNoteKind` enum, while necessary, did NOT fully resolve the reviewer's concern: entries with `Recommendation=NoOp && ConsistencyNoteKind=NaturalLanguageEmbeddingMissing` were still emitted into the `Discrepancies` list. Consumers filtering `Discrepancies.Where(d => d.Recommendation != NoOp)` missed real NL-only gaps. Fix: added additive `ConsistencyVerificationResult.Notes { get; init; } = []` property (init-only on positional record — wire-compat). `ConsistencyVerificationWorkflow` now routes `Recommendation=NoOp + ConsistencyNoteKind != None` entries to `Notes`; multi-axis discrepancies keep the NL note as supplementary context on the `Discrepancies` entry. Consumer contract: `Discrepancies = action-required`, `Notes = informational observations`. Updated existing test `RunAsync_IndexedNaturalLanguageMissing_BecomesDiscrepancyWithNote` → `RunAsync_IndexedNaturalLanguageMissing_RoutesToNotesNotDiscrepancies`; added `RunAsync_MultiAxisGapWithNlNote_RoutesToDiscrepanciesNotNotes` complement. 10/10 `ConsistencyVerificationWorkflowTests` green.
    - **Build / test verification.** Full solution `dotnet build Hexalith.Memories.slnx` → 0W/0E in 15s. Focused regression run over Consistency* + Hosting + Ingestion.IngestionInputMetadataComparerTests + IngestionWorkflow* + VerifyConsistencyActivity* + ConsistencyInspectionService* filters: **126/126 green** in 55s. No pre-existing Story 9.1/8.5 failures touched (those remain as documented in Session 3+ notes).
    - Story **transitions back to `review`** after closing D2/D6/D7. Remaining deferred items in the committed-branch review section (P8/P9/P10/P11/P14/P15/P16/P18/P19/P21/P23) stay explicitly marked deferred with inline rationale — they're intentional follow-up work scoped to later story passes, not active blockers on 9.2 AC completion. The committed-branch review overall now has 6 decisions resolved as-applied, 3 decisions resolved as-deferred with triggers, and 10 safe patches already landed in the 2026-04-24 batch — no open decision-items remain.
...
**Session 6 changes (2026-04-25 committed-branch D2/D6/D7 closure):**

- Modified: `src/Hexalith.Memories.Server/Hosting/WorkflowReplaySafetyHostedService.cs` (D2 — cached `MetadataField` FieldInfo; drill `state._metadata.Name`; startup probe emits Critical 9173 with reason on first reflection failure; consolidated `LogSidecarUnreachable`/`LogReflectionProbeFailed` into single `LogGateFailedOpen(reason, exception)` helper for LoggerMessage source-gen compatibility)
- Modified: `src/Hexalith.Memories.Contracts/V1/IngestionInput.cs` (D6 — pinned `StringComparer.Ordinal` on auto-backed default + init-rebuilds-with-Ordinal normalization)
- Modified: `src/Hexalith.Memories.Contracts/V1/IndexInput.cs` (D6 — same pinning as IngestionInput)
- Modified: `src/Hexalith.Memories.Server/Workflows/IngestionWorkflow.cs` (D6 — `BuildIndexMetadata` copy now passes `StringComparer.Ordinal` explicitly)
- Modified: `src/Hexalith.Memories.Contracts/V1/ConsistencyVerificationResult.cs` (D7 — additive `Notes` init-only property for the structural split)
- Modified: `src/Hexalith.Memories.Server/Workflows/ConsistencyVerificationWorkflow.cs` (D7 — routes `Recommendation=NoOp + ConsistencyNoteKind != None` entries to `Notes` instead of `Discrepancies`)
- Modified: `tests/Hexalith.Memories.Server.Tests/Hosting/WorkflowReplaySafetyHostedServiceTests.cs` (D2 guard test `WorkflowStateMetadataField_StillExists_OnCurrentSdkSurface`)
- Created: `tests/Hexalith.Memories.Server.Tests/Ingestion/IngestionInputMetadataComparerTests.cs` (D6 — 4 pinned-comparer tests)
- Modified: `tests/Hexalith.Memories.Server.Tests/Workflows/ConsistencyVerificationWorkflowTests.cs` (D7 — renamed/rewrote `IndexedNaturalLanguageMissing_BecomesDiscrepancyWithNote` → `…_RoutesToNotesNotDiscrepancies`; added `MultiAxisGapWithNlNote_RoutesToDiscrepanciesNotNotes` complement)
...
- [x] [Review][Decision] D2. `WorkflowReplaySafetyHostedService.TryGetWorkflowName` reflects on `Name` property → **(b) startup probe — APPLIED (Session 6, 2026-04-25).** `WorkflowState` does NOT expose `Name` publicly; reflection on the public surface was silently returning null, which meant `ShouldCountWorkflow` never matched any workflow and the gate always passed with `count=0`. Fix: cached `FieldInfo MetadataField = typeof(WorkflowState).GetField("_metadata", NonPublic | Instance)` + drill through `_metadata.Name` (verified via reflection probe: `WorkflowMetadata.Name` is public). Added startup probe in `TryCountInFlightAsync`: on the first in-flight instance, if reflection returns null the gate emits Critical `9173` with reason `workflow-name-reflection-null` and fails open (operator alert rather than silent no-op). The existing Critical `9173` sidecar-unreachable path was consolidated into a single `LogGateFailedOpen(reason, exception)` helper to keep the event ID unique per LoggerMessage source generator. Guard test `WorkflowStateMetadataField_StillExists_OnCurrentSdkSurface` asserts the field + type + `WorkflowMetadata.Name` property are present so a future SDK drift fails fast at unit-test time. Build 0W/0E; 5/5 `WorkflowReplaySafetyHostedServiceTests` green. [src/Hexalith.Memories.Server/Hosting/WorkflowReplaySafetyHostedService.cs:~30,~110-175,~225-230 + tests/Hexalith.Memories.Server.Tests/Hosting/WorkflowReplaySafetyHostedServiceTests.cs:~13-40]
...
- [x] [Review][Decision] D6. Metadata case-sensitive lookup → **(a) pin comparer — APPLIED (Session 6, 2026-04-25).** `IngestionInput.Metadata` and `IndexInput.Metadata` auto-backed defaults now construct `new Dictionary<string, MetadataField>(StringComparer.Ordinal)`; the `init` accessor rebuilds with Ordinal even if the caller passes a looser comparer (so `StringComparer.OrdinalIgnoreCase`-backed dictionaries from upstream code are normalized at the contract boundary). `BuildIndexMetadata` in `IngestionWorkflow` now copies with explicit `StringComparer.Ordinal` (the parameterless `new Dictionary<…>(IDictionary<…>)` overload does NOT carry the source's comparer — common footgun). Audited: `CloudEventToIngestionInputMapper` + `MemoriesClient` + `QuickstartSampleFlow` already use Ordinal — no further call-site edits needed. Guard tests `IngestionInputMetadataComparerTests` verify the comparer on default, null-init, case-insensitive-source, and `IndexInput.Metadata` default paths (4/4 green). [src/Hexalith.Memories.Contracts/V1/IngestionInput.cs, IndexInput.cs + src/Hexalith.Memories.Server/Workflows/IngestionWorkflow.cs:~770 + tests/Hexalith.Memories.Server.Tests/Ingestion/IngestionInputMetadataComparerTests.cs]
- [x] [Review][Decision] D7. NL discrepancy with `Recommendation=NoOp` → **(c) split discrepancies/notes — APPLIED (Session 6, 2026-04-25).** Re-verification confirmed Session 5's `ConsistencyNoteKind` enum, while necessary, does NOT fully resolve the concern: NL-only-gap entries (three core axes fine, `Recommendation=NoOp`, `ConsistencyNoteKind=NaturalLanguageEmbeddingMissing`) were still emitted into the `Discrepancies` list. Consumers filtering `Discrepancies.Where(d => d.Recommendation != NoOp)` silently missed real NL gaps. Fix: added additive `ConsistencyVerificationResult.Notes` property (default empty list, init-only — wire-compat for positional record). `ConsistencyVerificationWorkflow` now routes entries with `Recommendation=NoOp && ConsistencyNoteKind != None` to `Notes`; multi-axis discrepancies carry the NL note as supplementary context on the `Discrepancies` entry as before. Consumer contract: `Discrepancies` = action-required, `Notes` = informational observations. Guard tests `RunAsync_IndexedNaturalLanguageMissing_RoutesToNotesNotDiscrepancies` + `RunAsync_MultiAxisGapWithNlNote_RoutesToDiscrepanciesNotNotes` + pre-existing 8 tests = 10/10 green. [src/Hexalith.Memories.Contracts/V1/ConsistencyVerificationResult.cs + src/Hexalith.Memories.Server/Workflows/ConsistencyVerificationWorkflow.cs:~95-165 + tests/Hexalith.Memories.Server.Tests/Workflows/ConsistencyVerificationWorkflowTests.cs:~280-345]
...
#### Uncommitted review (2026-04-24, current working tree scope)

Raw findings: 24. After normalization + dedup + verification: 0 decision-needed / 7 patch / 0 defer / 8 dismissed as noise or already-handled design trade-offs.

- [x] [Review][Patch] Queued NL degraded state still bypasses informational `Notes` routing [src/Hexalith.Memories.Server/Workflows/ConsistencyVerificationWorkflow.cs:103]
- [x] [Review][Patch] `InconsistentCount` and `TotalDiscrepancyCount` still include note-only results [src/Hexalith.Memories.Server/Workflows/ConsistencyVerificationWorkflow.cs:165]
- [x] [Review][Patch] `Notes` uses an independent 10k cap with no truncation accounting [src/Hexalith.Memories.Server/Workflows/ConsistencyVerificationWorkflow.cs:147]
- [x] [Review][Patch] Human/table CLI formatters ignore `ConsistencyVerificationResult.Notes` [src/Hexalith.Memories.Cli/Output/Formatters/ConsistencyVerificationResultHumanFormatter.cs:25]
- [x] [Review][Patch] D6 comparer changes still lack replay/serialization/workflow-path regression tests [tests/Hexalith.Memories.Server.Tests/Ingestion/IngestionInputMetadataComparerTests.cs:1]
- [x] [Review][Patch] Replay-safety gate only probes null workflow names on the first active instance [src/Hexalith.Memories.Server/Hosting/WorkflowReplaySafetyHostedService.cs:145]
- [x] [Review][Patch] `RunAsync_MultiAxisGapWithNlNote_RoutesToDiscrepanciesNotNotes` does not exercise NL-note propagation [tests/Hexalith.Memories.Server.Tests/Workflows/ConsistencyVerificationWorkflowTests.cs:323]

**Applied-patch validation (2026-04-24):** `dotnet test tests/Hexalith.Memories.Server.Tests/Hexalith.Memories.Server.Tests.csproj --filter "FullyQualifiedName~ConsistencyVerificationWorkflowTests|FullyQualifiedName~WorkflowReplaySafetyHostedServiceTests|FullyQualifiedName~IngestionInputMetadataComparerTests|FullyQualifiedName~IngestionWorkflowTests"` → 57/57 green. `dotnet test tests/Hexalith.Memories.Contracts.Tests/Hexalith.Memories.Contracts.Tests.csproj --filter "FullyQualifiedName~IngestionInputSerializationTests|FullyQualifiedName~IndexInputSerializationTests"` → 12/12 green. `dotnet test tests/Hexalith.Memories.Cli.Tests/Hexalith.Memories.Cli.Tests.csproj --filter "FullyQualifiedName~ConsistencyVerifyCommandTests"` → 8/8 green.
...
[Patch]: 7 applied automatically (2026-04-24)

#### Session 6 re-review (2026-04-25, `e603a76^..e603a76`, D2/D6/D7 fix scope)

Three parallel adversarial layers (Blind Hunter + Edge Case Hunter + Acceptance Auditor) over the 1,219-line Session 6 commit (15 files, +750 / −80). Raw findings: 48. After dedup + verification + triage: 6 decision-needed / 17 patch / 9 defer / 3 dismissed.

**Decision-needed (resolved 2026-04-25 — "do best" autonomous resolution)**

- [x] [Review][Decision] S6-D1. Shared 10K cap → **(c) independent caps — APPLIED.** Added `public const int MaxNoteEntries = 10_000;` alongside the existing `MaxDiscrepancyEntries`; routing now checks `notes.Count < MaxNoteEntries` and `discrepancies.Count < MaxDiscrepancyEntries` independently. Worst-case payload ceiling doubles (still well within DAPR workflow state per-instance budget) but actionable items are never evicted by informational notes. [src/Hexalith.Memories.Server/Workflows/ConsistencyVerificationWorkflow.cs:42-46,154-168]
- [x] [Review][Decision] S6-D2. Single null reflection fails gate open globally → **(b) best-effort — APPLIED.** Per-instance null-name routes to new Warning EventId `9175 LogWorkflowNameUnreadable` and the loop continues. Only when EVERY active workflow had a null name does the gate escalate to Critical `9173 workflow-name-reflection-null` and fail open. The old `ShouldFailOpenForUnreadableWorkflowName` helper + its 4-scenario test are deleted (helper had no remaining production caller). [src/Hexalith.Memories.Server/Hosting/WorkflowReplaySafetyHostedService.cs:~120-200,~235-238]
- [x] [Review][Decision] S6-D3. `MemoryUnit.Metadata` Ordinal pinning → **(a) extend D6 — APPLIED.** Same fast-path init pattern as `IngestionInput` + `IndexInput`: rebuild with `StringComparer.Ordinal` unless the source already uses Ordinal (reference-equality check). [src/Hexalith.Memories.Contracts/V1/MemoryUnit.cs:28-39]
- [x] [Review][Decision] S6-D4. EventId 9173 overloaded → **(b) keep 9173, document — DEFERRED to runbook PR.** Adding new event IDs would churn dashboards/alerts. The structured `Reason` field (`workflow-name-reflection-null` / `sidecar-query-timeout` / `sidecar-query-exception`) already disambiguates. Re-open trigger: operator runbook + event-id bank update needs to land in the next docs PR — tracked as S6-FA in deferred-work.md (this re-review session).
- [x] [Review][Decision] S6-D5. `ConsistentCount` operator clarity → **(b) CLI formatter clarification — APPLIED.** Human formatter row now reads `consistent (incl. notes): {N}` so a careless reader does not double-count notes. Wire-level field name unchanged (no breaking contract change). [src/Hexalith.Memories.Cli/Output/Formatters/ConsistencyVerificationResultHumanFormatter.cs:27]
- [x] [Review][Decision] S6-D6. CLI `discrepancies` column semantic → **(a) revert to `Discrepancies.Count` — APPLIED.** In-payload count matches the `notes` column (also in-payload); operators detect truncation via `TotalDiscrepancyCount` / `TotalNoteCount` JSON fields and log events 8204 / 8210. [src/Hexalith.Memories.Cli/Output/Formatters/ConsistencyVerificationResultTableFormatter.cs:24-35]

**Patch (fix unambiguous)**

- [x] [Review][Patch] S6-P1. `NoteCount` semantics → **APPLIED.** Now `NoteCount = notes.Count` (truncated, in-payload). `TotalNoteCount = totalNoteCount` (un-truncated total). XML docs updated. New test `RunAsync_DiscrepancyCapExceeded_TruncatesDiscrepanciesNotNotes` asserts the new pair. [src/Hexalith.Memories.Server/Workflows/ConsistencyVerificationWorkflow.cs:218-220 + ConsistencyVerificationResult.cs:48-60]
- [x] [Review][Patch] S6-P2. `inconsistentCount` invariant restored → **APPLIED.** Now `inconsistentCount = totalUnits - consistentCount`. By construction this equals `totalDiscrepancyCount` and preserves `ConsistentCount + InconsistentCount = TotalUnits`. [src/Hexalith.Memories.Server/Workflows/ConsistencyVerificationWorkflow.cs:179]
- [x] [Review][Patch] S6-P3. Eager startup probe for `MetadataField is null` → **APPLIED.** `StartingAsync` now checks the cached `MetadataField` at the very top of the gate and emits Critical 9173 `metadata-field-missing` immediately if the SDK drifted, independent of in-flight workflow count. [src/Hexalith.Memories.Server/Hosting/WorkflowReplaySafetyHostedService.cs:67-74]
- [x] [Review][Patch] S6-P4. `Notes` init null-safe → **APPLIED.** Lazy-init pattern matching `IngestionInput.Metadata` style: `get => field ?? [];  init => field = value ?? [];`. [src/Hexalith.Memories.Contracts/V1/ConsistencyVerificationResult.cs:74-78]
- [x] [Review][Patch] S6-P5. `IndexInput.cs` MIT header → **APPLIED.** [src/Hexalith.Memories.Contracts/V1/IndexInput.cs:1-4]
- [x] [Review][Patch] S6-P6. Revert log 8204 message text → **APPLIED + split into 8204/8210.** With independent caps, EventId 8204 retains the original `DiscrepancyListTruncated` prefix (operator dashboards keyed on this string keep firing); a new `EventId 8210 NotesListTruncated` covers notes truncation. Truncation logic emits each independently. [src/Hexalith.Memories.Server/Workflows/ConsistencyVerificationWorkflow.cs:241-260]
- [ ] [Review][Patch] S6-P7. Log 8201 `Recommendation` field overload → **DEFERRED.** Adding `EntryKind` to the LoggerMessage signature is a wire-shape change for log consumers; this should land alongside the operator runbook update for D4. Tracked as S6-FB in deferred-work.md.
- [ ] [Review][Patch] S6-P8. `MemoriesJsonContext` round-trip test for new init-only properties → **DEFERRED to test-coverage follow-up.** Existing CLI JSON round-trip already exercises these properties end-to-end; the standalone source-gen contract test is helpful but not blocking. Tracked as S6-FC.
- [ ] [Review][Patch] S6-P9. `TryGetWorkflowName` end-to-end test → **DEFERRED.** Constructing a real `WorkflowState` instance requires test-double scaffolding the SDK does not document. Tracked as S6-FD; gated on Story 9.x integration-test infrastructure.
- [x] [Review][Patch] S6-P10. Cache `Name` PropertyInfo → **APPLIED.** New `Lazy<PropertyInfo?> NamePropertyAccessor` caches the property lookup off the cached `MetadataField.FieldType`. [src/Hexalith.Memories.Server/Hosting/WorkflowReplaySafetyHostedService.cs:51-56,229-244]
- [x] [Review][Patch] S6-P11. Hoist `isNoteOnlyObservation` → **APPLIED.** Computed once at the top of the loop body; reused for early-continue, totalNoteCount increment, and routing. [src/Hexalith.Memories.Server/Workflows/ConsistencyVerificationWorkflow.cs:117]
- [x] [Review][Patch] S6-P12. Init accessor fast-path for already-Ordinal dictionaries → **APPLIED.** Switch expression returns the existing Dictionary instance when `ReferenceEquals(value.Comparer, StringComparer.Ordinal)`, preserving reference identity for `with`-expression round-trips. [src/Hexalith.Memories.Contracts/V1/IngestionInput.cs:35-43, IndexInput.cs:39-47]
- [ ] [Review][Patch] S6-P13. Human formatter "notes: none" layout → **DEFERRED.** Cosmetic; minimal operator impact. Tracked as S6-FE.
- [ ] [Review][Patch] S6-P14. Stronger table-test assertions → **DEFERRED.** Existing tests pass against the corrected D6 column semantic; stronger assertions are a test-quality follow-up. Tracked as S6-FF.
- [ ] [Review][Patch] S6-P15. `docs/dev/consistency.md` Notes-split documentation → **DEFERRED to runbook PR (alongside D4 9173 multi-reason documentation).** Tracked as S6-FA.
- [x] [Review][Patch] S6-P16. Drop `FullName` assertion → **APPLIED.** SDK-drift sentinel keeps the `_metadata` field-presence + `Name`-property assertions; namespace relocations no longer break the test. [tests/Hexalith.Memories.Server.Tests/Hosting/WorkflowReplaySafetyHostedServiceTests.cs:34-37]
- [ ] [Review][Patch] S6-P17. Rename `MaxDiscrepancyEntries` → **OBSOLETED by S6-D1.** Independent caps means the original name is now correct (caps discrepancies only); the new `MaxNoteEntries` constant covers the notes side. No rename needed.

**Applied-patch validation (2026-04-25):** `dotnet test tests/Hexalith.Memories.Server.Tests/Hexalith.Memories.Server.Tests.csproj --filter "FullyQualifiedName~ConsistencyVerificationWorkflowTests|FullyQualifiedName~WorkflowReplaySafetyHostedServiceTests|FullyQualifiedName~IngestionInputMetadataComparerTests|FullyQualifiedName~IngestionWorkflowTests"` → 57/57 green. `dotnet test tests/Hexalith.Memories.Contracts.Tests/Hexalith.Memories.Contracts.Tests.csproj --filter "FullyQualifiedName~IngestionInputSerializationTests|FullyQualifiedName~IndexInputSerializationTests|FullyQualifiedName~ConsistencyContractSerializationTests"` → 31/31 green. `dotnet test tests/Hexalith.Memories.Cli.Tests/Hexalith.Memories.Cli.Tests.csproj --filter "FullyQualifiedName~ConsistencyVerifyCommandTests"` → 8/8 green. Server build 0W/0E.

**Defer (tracked in deferred-work.md)**

- [x] [Review][Defer] S6-F1. Comparer-rebuild guard test or analyzer that scans the server project for `new Dictionary<string, MetadataField>(...)` calls without an explicit comparer (D6 future-regression sentinel) — deferred (large scope, not blocking). (Source: A8.)
- [x] [Review][Defer] S6-F2. `LogGateFailedOpen` outer-caller side-comment contract — replace the "TryCountInFlightAsync already logged" comment with an explicit "log on null return" outer log so future refactors of the inner method cannot silently swallow failures. Deferred (refactor scope). (Source: B8.)
- [x] [Review][Defer] S6-F3. `BuildIndexMetadata` test currently uses `BindingFlags.NonPublic | BindingFlags.Static` reflection on a private static. Mark the method `internal` + add `InternalsVisibleTo` instead. Deferred (cosmetic). (Source: B12.)
- [x] [Review][Defer] S6-F4. New per-note `LogDiscrepancyDetected` emission inflates log volume for healthy tenants under NL rollout — split into `LogDiscrepancyDetected` (Warn) + `LogConsistencyNoteObserved` (Info/Debug). Deferred — tune after rollout volume is observed. (Source: B15.)
- [x] [Review][Defer] S6-F5. OCE `when` filter at `WorkflowReplaySafetyHostedService.cs:177-181` only catches per-call timeouts when outer cancellation is NOT requested; concurrent host shutdown + per-call timeout swallows the diagnostic. Deferred — narrow edge case during shutdown. (Source: A11.)
- [x] [Review][Defer] S6-F6. Static cctor reflection lookup `typeof(WorkflowState).GetField(...)` throws `TypeInitializationException` if `WorkflowState` cannot be loaded (e.g., Dapr.Workflow assembly missing/version-mismatched at runtime), bypassing the 9173 path entirely. Move to `Lazy<FieldInfo?>` with try/catch + 9173 emission. Deferred — runtime assembly-loading failures rare. (Source: E6.)
- [x] [Review][Defer] S6-F7. Free-text-only `ConsistencyNote` (i.e., `ConsistencyNoteKind = None` AND non-empty `ConsistencyNote`) routes to Notes with `kind = None`, which consumers cannot pattern-match — extend `BuildConsistencyNoteKind` fallback to return a sentinel like `Other`. Deferred — no current activity emits this combination. (Source: E9.)
- [x] [Review][Defer] S6-F8. `ListInstanceIdsAsync` enumeration page-size hardcoded to 100 with no inner-loop deadline check; under hundreds of in-flight instances, a single `TryCountInFlightAsync` call can outlive the documented 5-min `TotalTimeout`. Add deadline check inside the inner foreach. Deferred — operationally rare; pre-existing structure. (Source: E16.)
- [x] [Review][Defer] S6-F9. `BatchSize=1` accumulator test for the notes/discrepancies routing across batch boundaries is missing — risk of carry-over off-by-one not caught by the `[10_001]` test. Deferred — accumulator path appears correct on inspection. (Source: B18.)

**Dismiss**

- [x] [Review][Dismiss] S6-X1. "Notes init-only property may not serialize via `MemoriesJsonContext`" — false positive. Source-gen for records with init-only properties works for public types registered via `[JsonSerializable(typeof(ConsistencyVerificationResult))]`; the existing CLI round-trip test exercises this end-to-end. (S6-P8 still tracks the dedicated round-trip test gap.) (Source: B14.)
- [x] [Review][Dismiss] S6-X2. "Test class `IngestionInputMetadataComparerTests` not `sealed`" — convention drift exists project-wide (sibling test classes also unsealed); not a Session 6 regression. (Source: A7.)
- [x] [Review][Dismiss] S6-X3. "`BuildEntrySummary` private static helper missing XML doc" — `private` members exempt from the project's mandatory-doc rule (which scopes to public/protected/internal). (Source: A13.)

[Decision]: 6 resolved (D1=c, D2=b, D3=a, D4=b/deferred to runbook, D5=b, D6=a)

[Patch]: 11 applied (P1, P2, P3, P4, P5, P6, P10, P11, P12, P16, S6-D1/D3 production code) + 5 deferred to follow-up (S6-FA through S6-FF) + 1 obsoleted (P17)

[Defer]: 9 + 6 (S6-FA…S6-FF from this resolution) logged in deferred-work.md

### Review Findings

- [x] [Review][Decision] Mixed readable/unreadable active workflows can still false-drain the replay-safety gate — **APPLIED (conservative delay).** AC #11 says `WorkflowReplaySafetyHostedService` enforces quiescing until in-flight `IngestionWorkflow` instances clear. Current best-effort logic skipped every active instance whose workflow name was unreadable and only failed open when *all* active instances were unreadable. If one active unreadable instance was actually an `IngestionWorkflow` and any other active workflow had a readable non-ingestion name, `TryCountInFlightAsync` returned `0`, `StartingAsync` logged drained, and startup proceeded through the exact in-flight replay hazard the gate exists to prevent. Fix: unreadable active workflows now log Warning 9175 and are counted as in-flight until they clear or the existing 5-minute timeout fires. Added `ShouldBlockForUnreadableWorkflowName_ActiveStatesReturnTrue` guard. Evidence: `src/Hexalith.Memories.Server/Hosting/WorkflowReplaySafetyHostedService.cs:172-183`; story AC #11 requires the code-level gate to delay until in-flight `IngestionWorkflow` instances clear.
- [x] [Review][Patch] Human formatter labels truncated payload count as note-only units [src/Hexalith.Memories.Cli/Output/Formatters/ConsistencyVerificationResultHumanFormatter.cs:29] — **APPLIED.** `ConsistencyVerificationResult.NoteCount` was changed to mean `Notes.Count` (the truncated in-payload count), while `TotalNoteCount` is the un-truncated count. The human output still labeled `NoteCount` as `note-only units`, then printed `note records` from `Notes.Count`, so under note truncation it both underreported the real note-only unit count and duplicated the same payload count under two labels. Fix: human output now uses `TotalNoteCount` for `note-only units`; added `HumanFormatter_NoteOnlyUnits_UsesUntruncatedTotal`.

**Applied-patch validation (2026-04-25):** `dotnet test .\tests\Hexalith.Memories.Server.Tests\Hexalith.Memories.Server.Tests.csproj --no-restore --filter "FullyQualifiedName~ConsistencyVerificationWorkflowTests|FullyQualifiedName~WorkflowReplaySafetyHostedServiceTests|FullyQualifiedName~IngestionInputMetadataComparerTests|FullyQualifiedName~IngestionWorkflowTests" -v minimal` → 58/58 green. `dotnet test .\tests\Hexalith.Memories.Cli.Tests\Hexalith.Memories.Cli.Tests.csproj --no-restore --filter "FullyQualifiedName~ConsistencyVerifyCommandTests|FullyQualifiedName~ConsistencyVerificationResultFormatterTests" -v minimal` → 9/9 green.

### Review Findings

- [x] [Review][Patch] Replay-safety eager probe does not validate the cached workflow-name accessor [src/Hexalith.Memories.Server/Hosting/WorkflowReplaySafetyHostedService.cs:74] - **APPLIED.** `StartingAsync` now treats either missing `WorkflowState._metadata` or missing cached `WorkflowMetadata.Name` accessor as SDK drift, emits Critical 9173, and returns via fail-open before incorrectly reporting the gate drained.
- [x] [Review][Patch] Touched `MemoryUnit.cs` still lacks the required copyright header [src/Hexalith.Memories.Contracts/V1/MemoryUnit.cs:1] - **APPLIED.** Added the standard ITANEO MIT copyright header to match the project rule and neighboring contract files.

**Post-review patch validation (2026-04-25):** `dotnet test .\tests\Hexalith.Memories.Contracts.Tests\Hexalith.Memories.Contracts.Tests.csproj --no-restore --filter "FullyQualifiedName~IngestionInputSerializationTests|FullyQualifiedName~IndexInputSerializationTests|FullyQualifiedName~ConsistencyContractSerializationTests|FullyQualifiedName~MemoryUnitSerializationTests" -v minimal` → 39/39 green. `dotnet test .\tests\Hexalith.Memories.Server.Tests\Hexalith.Memories.Server.Tests.csproj --no-restore --filter "FullyQualifiedName~ConsistencyVerificationWorkflowTests|FullyQualifiedName~WorkflowReplaySafetyHostedServiceTests|FullyQualifiedName~IngestionInputMetadataComparerTests|FullyQualifiedName~IngestionWorkflowTests" -v minimal` → 58/58 green. `dotnet test .\tests\Hexalith.Memories.Cli.Tests\Hexalith.Memories.Cli.Tests.csproj --no-restore --filter "FullyQualifiedName~ConsistencyVerifyCommandTests|FullyQualifiedName~ConsistencyVerificationResultFormatterTests" -v minimal` → 9/9 green.
