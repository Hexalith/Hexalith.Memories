...
- Access telemetry lifecycle — current behavior is JSON-console emission plus optional OTLP export, with no repository-owned bounded lifecycle. [ADR 27.1-001](../../docs/dev/adr-27.1-001-access-telemetry-lifecycle.md) proposes a separate Redis 7.4 write-only access-telemetry workload: two StatefulSet data members on retained encrypted PVCs; three independently spread Sentinel members; TLS-only, default-deny network access; typed-state sanitization; a measured Server/Redis UTC gate; millisecond server-clock expiry; bounded key/index, lazy-free, and persisted-file reclamation; provider-specific filtering and non-blocking buffering; separate writer/lifecycle/inspection authorities; and explicit durability and rollback boundaries. The ADR is `Proposed`, not ratified: Stories 27.2 and 27.3 remain blocked until the all-nine-operation capacity recalculation is approved. `20.5-A41-ACCESS-TELEMETRY-RETENTION` remains open until implementation and Production-shaped evidence pass or an explicit accepted-debt disposition satisfies the recorded closure gate. This is bounded infrastructure telemetry, not tamper-evident, append-only, legally compliant, or certified audit retention.
