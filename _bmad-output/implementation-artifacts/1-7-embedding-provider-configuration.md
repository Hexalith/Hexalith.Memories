...
- [ ] Task 1: Create `TenantEmbeddingConfig` sealed record (AC: #1, #3)
  ...
  - [ ] 1.2 Register in `MemoriesJsonContext` for serialization
  - [ ] 1.3 Write serialization round-trip tests in `Contracts.Tests`

- [ ] Task 2: Create `EmbeddingProviderDefaults` static class (AC: #3)
  - [ ] 2.1 Create `EmbeddingProviderDefaults` in `Server/Ingestion/` with static factory methods returning default configs per known provider
  - [ ] 2.2 Google default: provider="google", model="gemini-embedding-001", dimensions=768, rateLimitPerMinute=1500
  - [ ] 2.3 Validation method: `Validate(TenantEmbeddingConfig)` that checks dimensions > 0, rateLimitPerMinute > 0 and <= MaxRateLimitPerMinute (3000 for Google — prevents one tenant from monopolizing shared API key), provider and model not null/empty, apiSecretKeyName matches `^[a-z0-9-]+$` (alphanumeric + hyphens only — prevents path traversal or cross-tenant key references)

- [ ] Task 3: Create `TenantConfigurationActor` for per-tenant config storage (AC: #1, #2, #4)
  - [ ] 3.1 Create `ITenantConfigurationActor` interface in `Server/Actors/` with methods: `GetEmbeddingConfigAsync()`, `SetEmbeddingConfigAsync(TenantEmbeddingConfig, bool forceReindex)` — no `GetFullConfigAsync()` (YAGNI per D9; GET endpoint can call `GetEmbeddingConfigAsync` directly)
  - [ ] 3.2 Create `TenantConfigurationActor` DAPR actor implementation — actor ID = tenant ID
  - [ ] 3.3 Store embedding config in actor state key `"embeddingConfig"`
  - [ ] 3.4 On `SetEmbeddingConfigAsync`: if provider/model/dimensions changed AND `forceReindex` is false, throw `EmbeddingConfigChangeException` warning reindex is required (AC #4). If `forceReindex` is true, save the new config AND set `ReindexRequired = true` on the stored `TenantEmbeddingConfig` (enforcement deferred to Epic 5, but the flag is tracked now)
  - [ ] 3.5 If no config exists (new tenant), return `EmbeddingProviderDefaults.Google()` as default
  - [ ] 3.6 Register actor in `Program.cs`
  - [ ] 3.7 Defensive deserialization in `GetEmbeddingConfigAsync()`: catch `JsonException` from `StateManager.GetStateAsync<TenantEmbeddingConfig>()`, log warning, return `EmbeddingProviderDefaults.Google()` — prevents permanent tenant lockout after schema evolution

- [ ] Task 4: Refactor `EmbeddingClient` to support configurable provider (AC: #2, #3)
  - [ ] 4.1 Extract constructor parameters to accept model, endpoint URL, and expected dimensions from config rather than hardcoded constants
  - [ ] 4.2 Add `output_dimensionality` field to request JSON payload (required by gemini-embedding-001 when using non-default dimensions)
  - [ ] 4.3 Update `ExpectedDimensions` to come from config, not const
  - [ ] 4.4 Update endpoint URL to be configurable (gemini-embedding-001 uses `/v1beta/` not `/v1/`)
  - [ ] 4.5 Keep API key retrieval via DAPR Secrets (secret key name from config)
  - [ ] 4.6 Update existing `EmbeddingClient` tests for new constructor shape
...
- [ ] Task 5: Refactor `GenerateEmbeddingActivity` to read tenant config (AC: #2)
  - [ ] 5.1 Add `IActorProxyFactory` call to `TenantConfigurationActor` to retrieve `TenantEmbeddingConfig`
  - [ ] 5.2 Pass config values (model, dimensions, endpoint URL) to `EmbeddingClient`
  - [ ] 5.3 Call `SetCeilingAsync(config.RateLimitPerMinute)` unconditionally on `EmbeddingRateLimiterActor` before `TryConsumeAsync()` — idempotent call, no need to read current state first (Occam's Razor: skip the read-compare-write cycle; actor can short-circuit internally if unchanged)
  - [ ] 5.4 Update `EmbeddingResult` to use provider/dimensions from config (not hardcoded)
  - [ ] 5.5 Update existing activity tests

- [ ] Task 6: Add REST endpoints for tenant embedding configuration (AC: #1, #4)
  - [ ] 6.1 `GET /api/tenants/{tenantId}/embedding-config` — returns current config including `reindexRequired` flag (partial FR45 satisfaction)
  - [ ] 6.2 `PUT /api/tenants/{tenantId}/embedding-config` — updates config; accepts `forceReindex` query param. On 409 Conflict (config change without forceReindex), return structured JSON error body: `{ "error": "EmbeddingConfigChangeRequired", "message": "...", "currentConfig": {...}, "proposedConfig": {...}, "affectedFields": ["dimensions"] }`
  - [ ] 6.3 Register endpoints in `Program.cs`

- [ ] Task 7: Update `EmbeddingInput`/`EmbeddingResult` records (AC: #2)
  - [ ] 7.1 `EmbeddingInput`: no change needed (TenantId is sufficient — activity reads config via actor)
  - [ ] 7.2 `EmbeddingResult`: Provider and Dimensions already dynamic — verify they come from config

- [ ] Task 8: Write comprehensive tests (all ACs)
  - [ ] 8.1 `TenantConfigurationActor` tests: get default config, set config, change detection, forceReindex bypass
  - [ ] 8.2 `EmbeddingProviderDefaults` tests: Google default values, validation logic
  - [ ] 8.3 `GenerateEmbeddingActivity` tests: reads tenant config, passes config to client, updates rate limiter ceiling
  - [ ] 8.4 Serialization tests for new records
  - [ ] 8.5 REST endpoint tests: PUT returns 409 Conflict when config changes affect provider/model/dimensions and `forceReindex=false`; GET returns default config for unconfigured tenant
  - [ ] 8.6 `EmbeddingClient` test: request JSON includes exact `"output_dimensionality"` field (snake_case, NOT camelCase) — Google API silently ignores unknown fields and returns 3072-dim vectors if name is wrong
  - [ ] 8.7 `EmbeddingClient` singleton test: two concurrent tenants with different `apiSecretKeyName` values both retrieve correct API keys from `ConcurrentDictionary` cache
  - [ ] 8.8 `EmbeddingProviderDefaults` validation tests: reject `apiSecretKeyName` with special chars, reject `rateLimitPerMinute` > 3000, reject dimensions <= 0
  - [ ] 8.9 `TenantConfigurationActor` test: `forceReindex=true` sets `ReindexRequired` flag on stored config
  - [ ] 8.10 `TenantConfigurationActor` test: corrupted actor state (bad JSON) returns default config instead of throwing — prevents permanent tenant lockout
