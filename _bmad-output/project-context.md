...
- **.NET 10 / C# 14** - all projects target `net10.0`; SDK is pinned by `global.json` to `10.0.400` with `rollForward=latestFeature`.
...
- **No infrastructure dependency in product code** - infrastructure is reached only through Dapr or Aspire; product projects (`Server`, `Cli`, `Mcp`, `Web`, `Client.Rest`) must not hardcode infrastructure endpoints/hosts/ports or construct infra clients — direct infra clients and connection lifecycle live only in boundary projects (`AppHost`, `Aspire`, `ServiceDefaults`, `Redis`, `EventStore`). Sanctioned exceptions: (1) direct search/vector/graph clients isolated in the Redis/EventStore boundary and Aspire-injected; (2) Dapr-platform env contracts (`DAPR_API_TOKEN`, `APP_API_TOKEN`, `DAPR_HTTP_PORT`, subscription-discovery topic env); (3) the CLI minimal direct-HTTP adapter with config-sourced endpoint defaults; (4) `EmbeddingProviderDefaultsOptions` property initializers as the single sanctioned home for embedding default endpoint/OIDC literals (guard covers `EmbeddingProviderDefaults` const/`static readonly` surfaces); (5) documented CLI tier-4 / local OTLP fallbacks in `docs/dev/cli-config.md`. See architecture Decision **D30** and **ADR-IDA-001** (the EventStore Dapr-state vs direct-Redis store split).
