...
2026-04-25 (session 3) — Review close-out resolved the final two review findings and completed Story 9.3. (1) `RedisObservedEventTypeStore.RecordObservationAsync` now restores the original hard-cap contract via one atomic Lua script (`SCARD` + `SISMEMBER` + conditional `SADD`/aggregates-key `EXPIRE` + `ZADD` + `HINCRBY` + 2× `EXPIRE`) in a single Redis round-trip: once the aggregates-index set reaches 1024, brand-new aggregate types are no longer admitted until the key naturally expires, while existing aggregate types continue to refresh the index TTL and the per-aggregate observation data still writes. The 9142 warning semantics now match the story/log contract again. Added focused guard tests for the atomic-script path plus the hard-cap warning/no-warning branches. (2) Added one-shot `EventStoreObservationStartupActivator : IHostedService` so `IEventIngestionTelemetry` is eagerly resolved during real host startup; this makes the adapter's constructor-time `9143 ObservationWritesConfigChanged(enabled:false)` emission genuinely happen at startup when the kill switch is disabled, instead of being deferred until first ingestion traffic. Added focused registration/startup-activator tests. Targeted regression slice green: 20/20 (`RedisObservedEventTypeStoreTests`, `EventIngestionTelemetryAdapterTests`, `EventStoreObservationStartupActivatorTests`). Story status review → done.
2026-04-25 (session 2) — Review-fix pass closed the last two unchecked Review Patches and returned the story to review. (1) `RedisObservedEventTypeStore.RecordObservationAsync` no longer issues pre-batch Redis round-trips: the prior implementation called `IDatabase.SetLengthAsync` + `IDatabase.SetContainsAsync` directly before the batch (1–2 RTTs). The new implementation pipelines `SCARD` + `SADD` + `ZADD` + `HINCRBY` + 3× `EXPIRE` into a single `IBatch.Execute()`, then inspects `cardinalityTask.Result` and `saddTask.Result` post-batch to emit the 9142 cardinality-cap warning. The cap becomes a soft cap (one excess admit per write at the boundary) but the 48h TTL remains the bound. Updated the two fail-open guard tests to inject failure via `db.When(d => d.CreateBatch(...)).Do(_ => throw ...)` instead of the now-unused `db.SetLengthAsync` direct call, and added `RecordObservationAsync_HappyPath_ShouldNotIssuePreBatchRoundTrips` as a positive guard. (2) `EventIngestionTelemetryAdapter` now emits `9143 ObservationWritesConfigChanged(enabled: false)` once at startup when `EventStoreIntegration:Observation:Enabled = false` from `appsettings.json` — previously the adapter only emitted on `OnChange` transitions, so a server that booted disabled produced no audit signal. The adapter is registered as `TryAddSingleton`, so the constructor-time emission runs exactly once per process. Added `KillSwitchInitiallyDisabled_ShouldEmitStartupConfigChangedLog_AC21A` and `KillSwitchInitiallyEnabled_ShouldNotEmitStartupConfigChangedLog` (using a small inline `CapturingTestLogger`) to pin both the AC #21 (a) emission and its negative case. Build clean (0W/0E). Focused regression: EventStore.Tests 82/82 green (prior 81/81 + the new pre-batch guard); Server.Tests Handlers + Adapter + Metrics 31/31 green (prior 29/29 + the two new startup tests); Cli.Tests Handlers + Formatter + Contract 14/14 green. Full Server.Tests 1488/1491 — same 3 pre-existing failures from Story 9.2 / 8.5 (`IngestionInputValidatorTests.Validate_Event_WithNullBytes_Throws`, `DocumentationCompletenessTests.EventStoreIntegrationDoc_HasRequiredSectionsAndKeyContent`, `ProvisionRediSearchActivityTests.RunAsync_IndexAlreadyExistsWithMatchingSchema_ShouldReturnTrue`). All seven Review Findings now resolved (5 Patch closed + 2 Defer carried forward as observation-store model-change follow-ups).
...
- [x] [Review][Patch] Accepted-path observation writes still perform pre-batch Redis round-trips [src/Hexalith.Memories.EventStore/RedisObservedEventTypeStore.cs:74]
- [x] [Review][Patch] Handler CLI formats and table-path options are not wired [src/Hexalith.Memories.Cli/CliServices.cs:111]
- [x] [Review][Patch] Observation-write telemetry contract is only partially implemented [src/Hexalith.Memories.Server/EventStoreIntegration/EventIngestionTelemetryAdapter.cs:80]
...
- [x] [Review][Patch] Aggregates-index cap is no longer enforced and should be restored to a hard stop [src/Hexalith.Memories.EventStore/RedisObservedEventTypeStore.cs:83]
- [x] [Review][Patch] Startup-disabled observation log is not guaranteed to emit at process startup [src/Hexalith.Memories.Server/EventStoreIntegration/EventIngestionTelemetryAdapter.cs:91]
