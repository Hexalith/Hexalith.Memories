// <copyright file="DeleteFalkorDbGraphActivity.cs" company="ITANEO">
...
/// <summary>Compensation activity that deletes a FalkorDB graph for a tenant.</summary>
public sealed partial class DeleteFalkorDbGraphActivity : WorkflowActivity<TenantProvisioningInput, bool>
{
    private readonly IConnectionMultiplexer _falkorDb;
    private readonly ILogger<DeleteFalkorDbGraphActivity> _logger;

    /// <summary>Initializes a new instance of the <see cref="DeleteFalkorDbGraphActivity"/> class.</summary>
    /// <param name="falkorDb">The FalkorDB connection multiplexer.</param>
    /// <param name="logger">The logger instance.</param>
    public DeleteFalkorDbGraphActivity(
        [FromKeyedServices("falkordb")] IConnectionMultiplexer falkorDb,
        ILogger<DeleteFalkorDbGraphActivity> logger)
    {
        _falkorDb = falkorDb;
        _logger = logger;
    }
    ...
    public override async Task<bool> RunAsync(WorkflowActivityContext context, TenantProvisioningInput input)
    {
        ...
        {
            // Graph may not exist — swallow gracefully
