...
- [x] [Review][Patch] Register concrete exact-profile capability probes; the production probe set is empty and the lifecycle remains permanently fail-closed [`src/Hexalith.Memories.AccessTelemetry/Program.cs`:46]
- [x] [Review][Patch] Renew capability evidence before `ValidUntilUtc`; the startup-only hosted probe lets writes continue on stale evidence [`src/Hexalith.Memories.AccessTelemetry/Capability/AccessTelemetryCapabilityProbeHostedService.cs`:18]
- [x] [Review][Patch] Resolve Production retention through Dapr configuration before validating lifecycle options [`src/Hexalith.Memories.AccessTelemetry/Program.cs`:17]
- [x] [Review][Patch] Reject undefined `RetentionSource` enum values even when Development retention is otherwise valid [`src/Hexalith.Memories.AccessTelemetry.Contracts/AccessTelemetryOptionsValidator.cs`:34]
- [x] [Review][Patch] Keep invalid lifecycle queue/options configuration from terminating Server business-host startup [`src/Hexalith.Memories.Server/Hosting/MemoriesServerServiceCollectionExtensions.cs`:508]
- [x] [Review][Patch] Grant the Server service account access to `access-telemetry-marker-key`; current RBAC only grants the lifecycle identity [`deploy/kubernetes/base/service-accounts-rbac.yaml`:37]
- [x] [Review][Patch] Derive `acceptedAtUtc` and absolute expiry from actor-verified time and configured retention instead of trusting writer-supplied timestamps [`src/Hexalith.Memories.AccessTelemetry/Lifecycle/AccessTelemetryLifecycleProcessor.cs`:49]
- [x] [Review][Patch] Require fresh trusted-clock validation for every lifecycle mutation, including each batch mutation, heartbeat, purge, inspection, and rotation transition [`src/Hexalith.Memories.AccessTelemetry/Lifecycle/AccessTelemetryLifecycleActor.cs`:52]
- [x] [Review][Patch] Bind attestations to authenticated caller identity and expected service/process/key epochs, and preserve replay protection across Server calls [`src/Hexalith.Memories.AccessTelemetry/Lifecycle/AccessTelemetryClockGate.cs`:40]
- [x] [Review][Patch] Compute a true overlapping strict-majority clock interval while tolerating bounded source failure and rejecting duplicate source identities [`src/Hexalith.Memories.AccessTelemetry.Clock/ClockAttestationService.cs`:41]
- [x] [Review][Patch] Load the clock signing authority through scoped Dapr secrets instead of a direct environment secret [`src/Hexalith.Memories.AccessTelemetry.Clock/Program.cs`:14]
- [x] [Review][Patch] Align expiry-entry serialization and both static/AppHost component query indexes so `expiryMinute` queries can find due records [`src/Hexalith.Memories.AccessTelemetry/Lifecycle/DaprAccessTelemetryStateStore.cs`:78]
- [x] [Review][Patch] Do not apply record expiry TTL to the expiry-index entry before authoritative delete-and-verify purge runs [`src/Hexalith.Memories.AccessTelemetry/Lifecycle/DaprAccessTelemetryStateStore.cs`:46]
- [x] [Review][Patch] Enforce configured purge limits/intervals, persist shard/minute cursors, and schedule bounded follow-up actor turns instead of looping unbounded [`src/Hexalith.Memories.AccessTelemetry/Lifecycle/AccessTelemetryLifecycleActor.cs`:125]
- [x] [Review][Patch] Update retained/purged accounting only for inserted records and verified deletions, not idempotent retries or failed absence checks [`src/Hexalith.Memories.AccessTelemetry/Lifecycle/AccessTelemetryLifecycleActor.cs`:88]
- [x] [Review][Patch] Compare expiry/hash before deletion so a stale expiry entry cannot delete a newer record reusing the same ID [`src/Hexalith.Memories.AccessTelemetry/Lifecycle/DaprAccessTelemetryStateStore.cs`:95]
- [x] [Review][Patch] Carry exact strong-read ETags and the required constant partition through transactional record/index mutations [`src/Hexalith.Memories.AccessTelemetry/Lifecycle/DaprAccessTelemetryStateStore.cs`:18]
- [x] [Review][Patch] Validate and cap heartbeat identities, generations, counts, and lease duration to prevent unbounded durable writer state [`src/Hexalith.Memories.AccessTelemetry/Lifecycle/AccessTelemetryLifecycleActor.cs`:106]
- [x] [Review][Patch] Connect staged marker-key rotation to Server generation-aware heartbeats, queue drain counts, acknowledgements, and activation calls [`src/Hexalith.Memories.Server/Telemetry/AccessTelemetryLifecycle/AccessTelemetryHeartbeatWorker.cs`:27]
- [x] [Review][Patch] Convert malformed canonical batches into bounded schema rejections instead of letting prevalidation exceptions escape actor invocation [`src/Hexalith.Memories.AccessTelemetry/Lifecycle/AccessTelemetryLifecycleActor.cs`:78]
- [x] [Review][Patch] Stop all subsequent writes after `record_id_conflict` establishes terminal lifecycle state [`src/Hexalith.Memories.AccessTelemetry/Lifecycle/AccessTelemetryLifecycleActor.cs`:86]
- [x] [Review][Patch] Reconcile persisted actor configuration after an explicit validated restart so epoch/profile changes can recover [`src/Hexalith.Memories.AccessTelemetry/Lifecycle/AccessTelemetryLifecycleActor.cs`:248]
- [x] [Review][Patch] Enforce the exact operation/event/outcome tuples, bounded query catalogs, error catalogs, correlation pairing, and operation-specific nullability in canonical validation [`src/Hexalith.Memories.AccessTelemetry.Contracts/AccessTelemetryCanonicalizer.cs`:227]
- [x] [Review][Patch] Reject unknown subtypes and wrong-typed logger state instead of normalizing them into legitimate actions or `not-applicable` values [`src/Hexalith.Memories.Server/Telemetry/AccessTelemetryLifecycle/AccessTelemetrySanitizer.cs`:212]
- [x] [Review][Patch] Preserve blank-tenant rejected-scope case evidence with the sentinel mapping instead of discarding it during case validation [`src/Hexalith.Memories.Server/Telemetry/AccessTelemetryLifecycle/AccessTelemetrySanitizer.cs`:98]
- [x] [Review][Patch] Make queue admission truly nonblocking under contention; the monitor lock can currently stall business logging [`src/Hexalith.Memories.Server/Telemetry/AccessTelemetryLifecycle/BoundedAccessTelemetryQueue.cs`:53]
- [x] [Review][Patch] Deserialize bounded terminal HTTP responses before success enforcement and stop retrying terminal configuration/conflict outcomes [`src/Hexalith.Memories.Server/Telemetry/AccessTelemetryLifecycle/DaprAccessTelemetryDeliveryClient.cs`:36]
- [x] [Review][Patch] Prevent non-prefix expiry and partial acceptance from permanently wedging FIFO acknowledgement [`src/Hexalith.Memories.Server/Telemetry/AccessTelemetryLifecycle/AccessTelemetryDeliveryWorker.cs`:44]
- [x] [Review][Patch] Honor configured batch byte/record limits and retry timing instead of hard-coded global maxima [`src/Hexalith.Memories.Server/Telemetry/AccessTelemetryLifecycle/AccessTelemetryDeliveryWorker.cs`:36]
- [x] [Review][Patch] Treat HTTP timeouts as retryable and the bounded shutdown-flush deadline as normal completion instead of terminating the hosted service [`src/Hexalith.Memories.Server/Telemetry/AccessTelemetryLifecycle/AccessTelemetryDeliveryWorker.cs`:56]
- [x] [Review][Patch] Count persisted/dropped records rather than successful batches in lifecycle metrics [`src/Hexalith.Memories.Server/Telemetry/AccessTelemetryLifecycle/AccessTelemetryDeliveryWorker.cs`:59]
- [x] [Review][Patch] Feed delivery, clock, conflict, purge, and no-data state into runtime health and preserve the actual inspector failure reason [`src/Hexalith.Memories.Server/Telemetry/AccessTelemetryLifecycle/AccessTelemetryLifecycleStatus.cs`:16]
- [x] [Review][Patch] Replace C2-C6 in-memory/direct-call/source-text checkpoints with hosted and routed Dapr/actor/clock/state evidence, including sanitizer branches, the HTTP UTC source, and hosted-worker retry/shutdown behavior [`tests/Hexalith.Memories.IntegrationTests/Telemetry/AccessTelemetryLifecycleIntegrationCheckpointTests.cs`:28]
- [x] [Review][Patch] Enable the local AppHost lifecycle/clock resources so the portable topology can actually be exercised [`src/Hexalith.Memories.AppHost/Program.cs`:253]
- [x] [Review][Patch] Revalidate retention/profile/epoch/key material after bootstrap changes instead of publishing a permanently static sanitizer/runtime decision [`src/Hexalith.Memories.Server/Telemetry/AccessTelemetryLifecycle/AccessTelemetryLifecycleBootstrapService.cs`:29]
...
| C2 - Server admission/delivery | Server implementation owner | `tests/Hexalith.Memories.Server.Tests/Telemetry/AccessTelemetryLifecycle/AccessTelemetryDeliveryCheckpointTests.cs`; run `dotnet exec tests/Hexalith.Memories.Server.Tests/bin/Release/net10.0/Hexalith.Memories.Server.Tests.dll -class Hexalith.Memories.Server.Tests.Telemetry.AccessTelemetryLifecycle.AccessTelemetryDeliveryCheckpointTests` | reviewed 2026-07-18 | complete; 27/27 green |
| C3 - Lifecycle actor and purge | Lifecycle implementation owner | `tests/Hexalith.Memories.AccessTelemetry.Tests/Lifecycle/LifecycleActorCheckpointTests.cs`; run `dotnet exec tests/Hexalith.Memories.AccessTelemetry.Tests/bin/Release/net10.0/Hexalith.Memories.AccessTelemetry.Tests.dll -class Hexalith.Memories.AccessTelemetry.Tests.Lifecycle.LifecycleActorCheckpointTests` | reviewed 2026-07-18 | complete; 12/12 green |
| C4 - Trusted clock | Clock implementation owner | `tests/Hexalith.Memories.AccessTelemetry.Tests/Clock/ClockAttestationCheckpointTests.cs`; run `dotnet exec tests/Hexalith.Memories.AccessTelemetry.Tests/bin/Release/net10.0/Hexalith.Memories.AccessTelemetry.Tests.dll -class Hexalith.Memories.AccessTelemetry.Tests.Clock.ClockAttestationCheckpointTests` | reviewed 2026-07-18 | complete; 13/13 green and threaded into writer/actor gates |
| C5 - Capability/security/observability gate | Platform implementation owner | `tests/Hexalith.Memories.AccessTelemetry.Tests/Capability/CapabilityAndObservabilityCheckpointTests.cs`; run `dotnet exec tests/Hexalith.Memories.AccessTelemetry.Tests/bin/Release/net10.0/Hexalith.Memories.AccessTelemetry.Tests.dll -class Hexalith.Memories.AccessTelemetry.Tests.Capability.CapabilityAndObservabilityCheckpointTests` | reviewed 2026-07-18 | complete; 20/20 green and exact-profile failures remain fail-closed |
| C6 - Portable composition and focused proof | Story 27.2 owner | `tests/Hexalith.Memories.IntegrationTests/Telemetry/AccessTelemetryLifecycleIntegrationCheckpointTests.cs` and `tests/Hexalith.Memories.IntegrationTests/Telemetry/AccessTelemetryAspireRoutedIntegrationTests.cs` plus reconciled story ledger/File List; run each focused class through the IntegrationTests runner | reviewed 2026-07-18 | complete; portable 8/8 and live Aspire/Dapr route 1/1 green; Production remains disabled pending Story 27.3 |
...
- 2026-07-18: Fresh post-review discovery used these exact commands for Server.Tests, IntegrationTests, and AccessTelemetry.Tests respectively: `DiffEngine_Disabled=true dotnet exec tests/Hexalith.Memories.Server.Tests/bin/Release/net10.0/Hexalith.Memories.Server.Tests.dll -list methods | rg '^Hexalith\.' | sort | tee >(sha256sum >&2) | wc -l`; `DiffEngine_Disabled=true dotnet exec tests/Hexalith.Memories.IntegrationTests/bin/Release/net10.0/Hexalith.Memories.IntegrationTests.dll -list methods | rg '^Hexalith\.' | sort | tee >(sha256sum >&2) | wc -l`; `DiffEngine_Disabled=true dotnet exec tests/Hexalith.Memories.AccessTelemetry.Tests/bin/Release/net10.0/Hexalith.Memories.AccessTelemetry.Tests.dll -list methods | rg '^Hexalith\.' | sort | tee >(sha256sum >&2) | wc -l`.
- 2026-07-18: Post-review discovery found Server.Tests 2,181 (review +12; cumulative +24; SHA-256 `62c935a6f13179d4f3e84912565279faffc0b84c4d223446a02be5c614659cd0`), IntegrationTests 279 (review +1; cumulative +9; SHA-256 `8af20c16b0d0a9f4b947357e232fb864f2bccba63aa33ccff3a81676979b9468`), and AccessTelemetry.Tests 39 methods (review +8; cumulative +39; SHA-256 `0e79e40274ddf051f891e5bdfae2f25d1cb4498fd82d865b78216081a2b99a14`). The review added 21 discovered methods and the cumulative story delta is 72, with no comparable-lane removals.
...
- 2026-07-18: Post-review focused evidence passed AccessTelemetry.Tests 57/57, Server delivery 27/27, combined Server decision/delivery 36/36, portable integration 8/8, live Aspire/Dapr routed integration 1/1, and preserved emission/privacy regression selection 74/74. The full Release solution build remained clean at 0 warnings/errors; both Kubernetes kustomizations and all five standalone Dapr YAML parses remained green.
- 2026-07-18: The live route exercised real AppHost resources, Dapr sidecars, independent signed clock attestation, runtime validation, signed heartbeat, fixed lifecycle actor inspection, and actual actor-state retrieval. All four access-telemetry resources became healthy; lifecycle health was `Healthy` with `None`, and the Server surface reached `Healthy`/`NoData`. The AppHost was stopped after proof and `aspire ps --non-interactive` reported no running AppHost.
- 2026-07-18: Review File List reconciliation carried forward the already reconciled 139-path development ledger and used ``story=_bmad-output/implementation-artifacts/27-2-bounded-retention-ttl-and-purge-implementation.md; comm -13 <(awk '/^### File List$/{inside=1;next} /^## Change Log$/{inside=0} inside && /^- `/{line=$0; sub(/^- `/, "", line); sub(/`$/, "", line); print line}' "$story" | sort) <(git status --porcelain=v1 --untracked-files=all | cut -c4- | rg -v '^references/' | sort)`` to add all 16 newly owned review paths. A raw diff from the development baseline also contains later unrelated trunk history, so it was not widened into Story 27.2. The cumulative list now matches 155/155 story paths: 154 cumulative owned paths plus the retained create-story scope-evidence artifact.
...
- 2026-07-18: Review-time runtime blockers—obsolete Redis query-index metadata, duplicate actor state stores, self-host ACL identity rejection without Sentry, a non-public actor interface, and a non-conflicting ETag capability probe—were reproduced, corrected, and cleared by the routed proof. The only remaining validation limitation is the historical offline Kubernetes API-discovery check above; it is environmental, does not block the green static manifests, and Story 27.3 still owns Production certification.
- 2026-07-18: Externally advanced root submodules `references/Hexalith.Builds`, `references/Hexalith.Commons`, `references/Hexalith.EventStore`, `references/Hexalith.FrontComposer`, `references/Hexalith.PolymorphicSerializations`, and `references/Hexalith.Tenants` were preserved and excluded from Story 27.2 scope.
...
- 2026-07-18: Applied all 36 adversarial code-review patches across contracts, clock trust, capability renewal, Dapr configuration/state semantics, actor mutation/purge/rotation, Server admission/delivery/health, least-privilege deployment, and hosted/routed verification. Fresh evidence is green, the cumulative File List is reconciled at 155/155, and the story is complete while Production certification remains in Story 27.3.
...
- `src/Hexalith.Memories.AccessTelemetry.Clock/DevelopmentAuthenticatedUtcSource.cs`
...
- `src/Hexalith.Memories.AccessTelemetry.Clock/Properties/launchSettings.json`
...
- `src/Hexalith.Memories.AccessTelemetry.Contracts/AccessTelemetryRuntimeValidationRequest.cs`
- `src/Hexalith.Memories.AccessTelemetry.Contracts/AccessTelemetryRuntimeValidationResponse.cs`
...
- `src/Hexalith.Memories.AccessTelemetry.Contracts/WriterHeartbeatRequest.cs`
- `src/Hexalith.Memories.AccessTelemetry.Contracts/WriterHeartbeatResponse.cs`
...
- `src/Hexalith.Memories.AccessTelemetry/Capability/DaprAccessTelemetryCapabilityProbe.cs`
...
- `src/Hexalith.Memories.AccessTelemetry/Lifecycle/AccessTelemetryDeleteStatus.cs`
...
- `src/Hexalith.Memories.AccessTelemetry/Lifecycle/AccessTelemetryLifecycleConfigurationHostedService.cs`
...
- `src/Hexalith.Memories.AccessTelemetry/Lifecycle/AccessTelemetryProcessorStatus.cs`
- `src/Hexalith.Memories.AccessTelemetry/Lifecycle/AccessTelemetryPurgeResult.cs`
- `src/Hexalith.Memories.AccessTelemetry/Lifecycle/AccessTelemetryRuntimeOptionsProvider.cs`
...
- `src/Hexalith.Memories.AccessTelemetry/Lifecycle/DaprLifecycleClockEvidenceProvider.cs`
...
- `src/Hexalith.Memories.AccessTelemetry/Lifecycle/ILifecycleClockEvidenceProvider.cs`
- `src/Hexalith.Memories.AccessTelemetry/Lifecycle/InMemoryAccessTelemetryStateStore.cs`
- `src/Hexalith.Memories.AccessTelemetry/Lifecycle/LifecycleClockEvidence.cs`
...
- `src/Hexalith.Memories.AccessTelemetry/Properties/launchSettings.json`
...
| 2026-07-18 | code-review | Applied all 36 adversarial patches, including exact-profile capability/configuration renewal, trusted clock and actor mutation enforcement, ETag/transaction/TTL/query/delete semantics, bounded purge/rotation and Server delivery/health behavior, local/Kubernetes authority fixes, and a self-contained live Aspire/Dapr routed proof. Production certification remains assigned to Story 27.3. | Review/cumulative method deltas: Server.Tests 2,169 -> 2,181, +12/+24; IntegrationTests 278 -> 279, +1/+9; AccessTelemetry.Tests 31 -> 39, +8/+39; total +21/+72, with no comparable-lane removals. Fresh hashes: Server `62c935a6f13179d4f3e84912565279faffc0b84c4d223446a02be5c614659cd0`; Integration `8af20c16b0d0a9f4b947357e232fb864f2bccba63aa33ccff3a81676979b9468`; AccessTelemetry `0e79e40274ddf051f891e5bdfae2f25d1cb4498fd82d865b78216081a2b99a14`. Executed cases: AccessTelemetry 57/57, Server delivery 27/27, Server decision/delivery 36/36, portable integration 8/8, routed integration 1/1, and preserved emission/privacy 74/74; full solution build 0 warnings/errors. | matched 155/155 cumulative story paths: 154 cumulative owned paths plus the retained create-story scope-evidence artifact. The exact status/awk/comm command and all exclusions are recorded in Debug Log References; all six externally advanced root submodules were preserved. |
