...
- [ ] Task 1b: Create shared index schema definitions (AC: #1)
    ...
    - [ ] 1b.4 Provisioning activities (Tasks 3.1, 3.2) will reference `IndexSchemaDefinitions` for identical schema -- no drift possible
...
- [ ] Task 3: Create provisioning activities (AC: #1, #2)
    ...
    - [ ] 3.3 Create `Server/Activities/Tenants/ProvisionFalkorDbActivity.cs` -- extends `WorkflowActivity<TenantProvisioningInput, bool>`. Creates a FalkorDB graph for the tenant by executing a no-op query: `RETURN 1`. FalkorDB creates the graph implicitly on any query -- `RETURN 1` is the simplest idempotent way to force graph creation without side effects (no phantom nodes, no cleanup needed). The graph ID = `tenantId`. Inject `IConnectionMultiplexer` (keyed "falkordb"). Use `new FalkorDB(db).QueryAsync(tenantId, "RETURN 1")` pattern. Return `true` on success. If graph already exists, the query is a no-op (idempotent)
...
- [ ] Task 4: Create TenantProvisioningWorkflow (AC: #1, #2, #3, #5)
    ...
    - [ ] 4.5 **Tenant registry access in workflow:** Workflows must NOT call external services directly -- only activities do I/O. The tenant-exists check and status updates must be activities too. Create:
        - `InitializeTenantRegistryActivity.cs` -- extends `WorkflowActivity<TenantProvisioningInput, TenantInfo>`. Atomically checks if tenant exists AND registers if not found. Uses `TenantRegistryService.TenantExistsAsync()` then `TenantRegistryService.RegisterTenantAsync()`. If tenant already exists, throws `InvalidOperationException("TENANT_ALREADY_EXISTS")`. This merges the existence check + registration into a single activity to eliminate the race condition window between two separate activities. Returns created `TenantInfo`
...
Task 1 -> 2 -> 3 -> 4 -> 5 -> 6 -> 7 -> 8-11 (tests in parallel). Contracts first (1), then registry service (2), then activities (3), then workflow (4), then DI registration (5), then endpoints (6, 7), then all tests.
...
FalkorDB creates graphs implicitly on first query. The provisioning activity executes `RETURN 1` -- a no-op that forces graph creation with zero side effects. No phantom nodes, no cleanup needed, fully idempotent. The `_SystemInit` create/delete pattern was considered but rejected: if the delete fails, it leaves a phantom node in the graph.
...
This keeps `ProvisionRedisVectorActivity` dependency-free (only needs `IConnectionMultiplexer`, no `DaprClient`). The `PUT /api/tenants/{tenantId}/embedding-config` endpoint already validates that changing dimensions requires reindex (409 Conflict), so provisioning with default dimensions is safe.
