...
- [ ] Task 3 - Prove expiry, purge, newer-record preservation, and cohort-attributable physical reclamation (AC: 1, 2)
  ...
  - [ ] Fault-inject interruption after record deletion but before expiry-index deletion, replace the lifecycle process/actor activation, and prove an idempotent retry removes the orphaned index, preserves newer records, and reports the cohort complete without duplicate purge accounting.
...
- [ ] Task 7 - Coordinate the evidence-backed A41 close-out without reopening historical work (AC: 3)
  - [ ] Create `_bmad-output/implementation-artifacts/tests/27-3-retention-verification-evidence.md` as the one canonical close-out matrix. It must index every C0-C6 packet, AC, exact environment identity, command, fresh assembly/source hash, immutable output/artifact hash, date, owner, and review decision. Every mandatory checkpoint must pass; any skipped, not-run, missing, stale, or unapproved result blocks closure.
  - [ ] Before mutation, run `python3 tools/verify-access-telemetry-lifecycle.py --checkpoint a41-inventory --evidence _bmad-output/implementation-artifacts/tests/27-3-retention-verification-evidence.md`. The verifier must derive the tracked candidate set from `git ls-files`, discover the exact residual ID plus every approved current alias/summary pattern, fail on an unclassified match or missing authoritative path, and persist the sorted path/hash manifest in the canonical matrix.
  - [ ] After C0-C5 and Task 8's complete pre-close validation pass, run `python3 tools/verify-access-telemetry-lifecycle.py --checkpoint a41-close-out --mode preflight --evidence _bmad-output/implementation-artifacts/tests/27-3-retention-verification-evidence.md`. Preflight must re-capture the live component/profile hash and require exact equality with the approved C1-C5 evidence, verify the exhaustive A41 inventory, require zero skips/not-runs/failures, and emit the exact allowed mutation set plus a recoverable pre-mutation snapshot/hash.
  - [ ] Apply only the preflight-approved mutation set, changing `20.5-A41-ACCESS-TELEMETRY-RETENTION` from `carried-forward` to a resolved evidence-citing disposition and the matching sprint action from `open` to `done` with the canonical matrix as their identical evidence anchor. Do not commit, publish, or claim closure until postflight succeeds.
  - [ ] Run the same verifier with `--mode postflight`; require exact mutation-set equality, all intended closed postconditions, no unintended A41/status/history changes, unchanged live profile identity, and reference integrity. On failure, use the recorded preflight snapshot/patch to restore the open disposition or complete the exact approved set, rerun postflight, and keep A41 open until the repository is internally consistent.
  - [ ] Reconcile current architecture, epics, canonical project context, telemetry guidance, audit finding summary, deferred register, and every path in the verifier-derived A41 inventory. Append non-destructive closure notes to historical Story 20.5/retrospective records only where repository convention requires; never rewrite their original evidence or reopen their `done` statuses.
...
- [ ] Task 8 - Validate the governed story, tests, evidence, and cumulative scope (AC: 1, 2, 3)
  ...
  - [ ] Complete this entire validation task and attach its immutable results to the canonical matrix before Task 7 preflight or any A41 mutation; Task numbering does not authorize close-out before this gate.
...
Every C# checkpoint command below is a two-stage command contract: first build the named test project from the reviewed source with `DOTNET_CLI_USE_MSBUILD_SERVER=0 dotnet build <project> --configuration Release --disable-build-servers -m:1 /nr:false -p:NuGetAudit=false -p:MinVerVersionOverride=1.0.0`; then hash the fresh assembly, discover the exact selected classes with `DiffEngine_Disabled=true dotnet exec <assembly> -list methods -noLogo`, and execute that same assembly with the table's selectors, `-parallel none -noLogo`. The evidence must prove every selector discovers at least one unique method, `Total` equals the selected discovery count and is greater than zero, and `Errors`, `Failed`, `Skipped`, and `Not Run` are all zero. A stale assembly, missing selector, zero discovery, count mismatch, or nonzero result fails the checkpoint even when the process exit code is zero.

Every Python checkpoint first runs the named `unittest` suite under `tests/tooling/access_telemetry_lifecycle`, records a strictly positive discovered/executed case count with zero failures/errors/skips, and then invokes the verifier against the same reviewed source/profile and canonical evidence matrix. Each evidence packet links into `_bmad-output/implementation-artifacts/tests/27-3-retention-verification-evidence.md`; checkpoint-local files are supporting packets, never competing closure anchors.

| Checkpoint | Accountable owner | Required evidence artifact and command | Review state | Completion state | Completion date |
| :--------- | :---------------- | :------------------------------------- | :----------- | :--------------- | :-------------- |
| C0 - Story 27.2 completion handoff | Story 27.2 owner + code reviewer | Reconciled Story 27.2 C1-C6/ledger/File List plus the IntegrationTests build/discovery/run contract for `-class Hexalith.Memories.IntegrationTests.Telemetry.AccessTelemetryLifecycleIntegrationCheckpointTests -parallel none -noLogo` | pending | blocked at creation: Story 27.2 is `in-progress`, runtime lane absent, C1-C6 pending | — |
| C1 - Exact Production adapter, capability, capacity, and security approval | Deployment adapter owner + Hexalith Platform Operations | `_bmad-output/implementation-artifacts/tests/27-3-adapter-profile-evidence.md`, indexed by the canonical matrix; run the Python contract and `python3 tools/verify-access-telemetry-lifecycle.py --checkpoint adapter-profile --evidence _bmad-output/implementation-artifacts/tests/27-3-retention-verification-evidence.md` | pending | blocked on C0 | — |
| C2 - Multi-writer, replacement, reminder, and single-fault durability proof | Story 27.3 evidence owner + security reviewer | `_bmad-output/implementation-artifacts/tests/27-3-production-lifecycle-evidence.md`, indexed by the canonical matrix; run the IntegrationTests build/discovery/run contract for `-class Hexalith.Memories.IntegrationTests.Telemetry.AccessTelemetryRetentionProductionVerificationTests -class Hexalith.Memories.IntegrationTests.Telemetry.AccessTelemetryLifecycleFaultRecoveryTests -parallel none -noLogo` | pending | blocked on C0-C1 | — |
| C3 - Expiry, purge, newer preservation, and physical reclamation | Deployment adapter owner + security reviewer | `_bmad-output/implementation-artifacts/tests/27-3-retention-reclamation-evidence.md`, indexed by the canonical matrix; run the AccessTelemetry.Tests build/discovery/run contract for `-class Hexalith.Memories.AccessTelemetry.Tests.Adapter.ProductionAdapterCapabilityAndReclamationEvidenceTests -parallel none -noLogo`, then the Python contract and `python3 tools/verify-access-telemetry-lifecycle.py --checkpoint retention-reclamation --evidence _bmad-output/implementation-artifacts/tests/27-3-retention-verification-evidence.md` | pending | blocked on C1-C2 | — |
| C4 - Failure, tenant/privacy, observability, and alerts | Test architect + Hexalith Platform Operations | `_bmad-output/implementation-artifacts/tests/27-3-failure-privacy-observability-evidence.md`, indexed by the canonical matrix; run the IntegrationTests build/discovery/run contract for `-class Hexalith.Memories.IntegrationTests.Telemetry.AccessTelemetryTenantPrivacyProductionTests -parallel none -noLogo`, then the Python contract and `python3 tools/verify-access-telemetry-lifecycle.py --checkpoint failure-privacy-observability --evidence _bmad-output/implementation-artifacts/tests/27-3-retention-verification-evidence.md` | pending | blocked on C0-C3 | — |
| C5 - Runbook and exact-adapter appendix | Technical writer + Hexalith Platform Operations + security reviewer | `docs/operations/access-telemetry-lifecycle.md` and `docs/operations/access-telemetry-adapter-production.md`, indexed by the canonical matrix; run the Server.Tests build/discovery/run contract for `-class Hexalith.Memories.Server.Tests.Deployment.AccessTelemetryOperationsContractTests -class Hexalith.Memories.Server.Tests.Deployment.OperationalRunbookSetTests -class Hexalith.Memories.Server.Tests.Telemetry.MemoriesDashboardTests -parallel none -noLogo` | pending | blocked on C1-C4 | — |
| C6 - Coordinated A41 close-out | Security remediation owner + planning owner | Canonical matrix `_bmad-output/implementation-artifacts/tests/27-3-retention-verification-evidence.md`; run the Task 7 inventory/preflight/postflight verifier contract and the Server.Tests build/discovery/run contract for `-class Hexalith.Memories.Server.Tests.Architecture.AccessTelemetryA41CloseOutTests -parallel none -noLogo` | pending | blocked on C0-C5 and complete Task 8 pre-close validation; no A41 mutation allowed | — |
...
| A41 deferred/action/current summaries | Deferred entry is `carried-forward`; sprint action is `open`; current summaries forbid premature closure | After C0-C5 and complete Task 8 pre-close validation, update only the verifier-approved mutation set to the canonical evidence anchor and require postflight parity | Epic 20/Story 20.5 history and `done` statuses; original approval/evidence chronology |
