...
- [ ] [Review][Patch] Fix `fake!` null race in `OllamaOidcFakeServer.StartAsync` [`tests/.../Fixtures/OllamaOidcFakeServer.cs:80-86`] — `ScriptedHttpServer.StartAsync` returns a listening server before `fake = new ...` runs; an inbound request between those statements dereferences null. Construct the instance first, then start the inner server.
- [ ] [Review][Patch] Snapshot `secrets.json` content before assigning `_secretsFilePath` [`tests/.../Fixtures/AspireIngestionPipelineFixture.cs:400-404`] — if `File.ReadAllText` throws after `_secretsFilePath` is set, `RestoreLocalDaprSecret` deletes the user's pre-existing secrets file (data loss).
- [ ] [Review][Patch] Handle non-object JSON root in `secrets.json` [`tests/.../Fixtures/AspireIngestionPipelineFixture.cs:407-410`] — `JsonNode.Parse(content)?.AsObject()` throws `InvalidOperationException` for arrays/scalars, not null. Use `as JsonObject ?? throw` with a clear error.
- [ ] [Review][Patch] Validate `EmbeddingProviderSecret.Name` against an identifier regex [`tests/.../Fixtures/EmbeddingProviderSecret.cs:11`] — current YAML interpolation in `CreateDaprConfigOverrideIfNeeded` is safe only because the single caller passes `memories-embedding-client-secret`. Enforce `^[A-Za-z0-9._-]+$` in the record so future callers cannot inject YAML.
- [ ] [Review][Patch] Restore typed JSON serialization in `ProvisionActiveTenantAsync` [`tests/.../Fixtures/AspireIngestionPipelineFixture.cs:295-303`] — change dropped `MemoriesJsonContext.Options` and switched to anonymous object, breaking AOT/source-gen consistency with the rest of the file. Extend the typed `TenantProvisioningInput` with optional `vectorDimensions` and pass `MemoriesJsonContext.Options`.
- [ ] [Review][Patch] Replace bearer-redaction substring assertions with token-value assertions [`tests/.../Fixtures/OllamaOidcFakeServerTests.cs:79`, `tests/.../Ingestion/OllamaEmbeddingEndToEndTests.cs:120`] — `evidence.ShouldNotContain("Bearer ")` is a coincidence (record `ToString()` never produces it); `logs.ShouldNotContain("Bearer " + "eyJ")` cannot detect leak of the opaque `example-bearer-token`. Assert against the actual token value (already done for the secret).
- [ ] [Review][Patch] Annotate operator-doc "Ollama local/no-auth" column as config-schema only [`docs/operations/embedding-providers.md:36-48`] — the matrix presents it as a first-class option but row `AuthMode` admits "not accepted by the current Ollama runtime dispatch". Add an explicit "(not yet runtime-accepted)" header annotation so operators do not configure a path the server rejects.
- [ ] [Review][Patch] `ResolveRepositoryRoot` must throw when `Hexalith.Memories.slnx` is not found [`tests/.../Fixtures/AspireIngestionPipelineFixture.cs:489-501`] — currently returns `Directory.GetCurrentDirectory()` silently; tests then write/restore a `secrets.json` in an arbitrary location.
- [ ] [Review][Patch] Wrap `JsonDocument.ParseAsync` in `HandleEmbedAsync` with try/catch returning 400 [`tests/.../Fixtures/OllamaOidcFakeServer.cs:184`] — non-JSON body throws and the fake returns 500 instead of asserting the contract violation.
- [ ] [Review][Patch] Validate `JsonValueKind` before `GetString()` in `HandleEmbedAsync` [`tests/.../Fixtures/OllamaOidcFakeServer.cs:191-193`] — `model`/`input` as null/number throws `InvalidOperationException`; reject with 400 instead.
- [ ] [Review][Patch] Dispose `_fakeServer` if `_fixture.InitializeAsync` throws [`tests/.../Ingestion/OllamaEmbeddingEndToEndTests.cs:30-37`] — xUnit does not call `DisposeAsync` after `InitializeAsync` failure; the Kestrel listener leaks until process exit.
- [ ] [Review][Patch] Distinguish workflow-failed from wait-expired in `WaitForSemanticHashAsync` timeout [`tests/.../Ingestion/OllamaEmbeddingEndToEndTests.cs:214-230`] — current code breaks the loop on terminal `Failed` status then throws a generic `TimeoutException`, masking the real failure cause.
- [ ] [Review][Patch] Guard `WorkflowReachedTerminalStatus` JSON parse [`tests/.../Ingestion/OllamaEmbeddingEndToEndTests.cs:233-260`] — non-JSON or empty body aborts the wait loop with an unhandled `JsonException` instead of polling.
- [ ] [Review][Patch] Scrub client secret/token from `WaitForSemanticHashAsync` timeout message [`tests/.../Ingestion/OllamaEmbeddingEndToEndTests.cs:227-230`] — `lastWorkflowPayload` and `logs` are embedded raw in the exception; if the workflow ever surfaces request payload or response headers, secrets leak into test output.
- [ ] [Review][Patch] Reject duplicate form values in `HandleTokenAsync` [`tests/.../Fixtures/OllamaOidcFakeServer.cs:152-155`] — duplicated `grant_type`/`client_id` produce ambiguous "malformed" errors; assert `StringValues.Count == 1` per required field with a distinct error.
