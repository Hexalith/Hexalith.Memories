...
Runs five sub-checks: Docker daemon (`docker ps`), .NET SDK (`dotnet --list-sdks` — asserts at least one SDK version 10.0.400 or newer), port availability (5000, 6379, 6380, 3500, 50001), OS platform (informational), and DAPR CLI (soft-fail — missing is OK).
...
- `No .NET SDK 10.0.400 or newer` → install the latest .NET 10 SDK from the link above.
