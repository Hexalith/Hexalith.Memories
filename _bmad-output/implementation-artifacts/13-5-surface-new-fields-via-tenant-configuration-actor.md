...
- no surface returns an actual OIDC `client_secret` value.
...
2. **AC2 - Invalid legacy fallback behavior is unchanged.** Corrupted JSON or validation-invalid stored config still logs a warning and returns `EmbeddingProviderDefaults.Google()` without writing replacement state during the read path.

3. **AC3 - Ollama config round-trips through actor state.** `SetEmbeddingConfigAsync(config, forceReindex: false)` persists a valid Ollama OIDC `TenantEmbeddingConfig` including all Story 13.4 fields, and a later `GetEmbeddingConfigAsync()` returns the same metadata values.
...
5. **AC5 - Breaking-change behavior uses the committed 13.4 contract.** Provider/model/dimensions changes still require `forceReindex`. Ollama-to-Ollama `BaseUrl` changes require `forceReindex` only if Story 13.4's `GetBreakingChangeFields(...)` includes `baseUrl`; `AuthMode`, `OidcTokenEndpoint`, `OidcClientId`, and `OidcScope` changes do not require reindex by themselves.

6. **AC6 - PUT/GET embedding-config surfaces carry new fields.** A valid `PUT /api/tenants/{tenantId}/embedding-config` body containing the Ollama fields is validated, passed to `SetEmbeddingConfigAsync(...)`, and the `200 OK` response includes the same `baseUrl`, `authMode`, `oidcTokenEndpoint`, `oidcClientId`, `oidcScope`, `apiSecretKeyName`, provider, model, dimensions, rate limit, and reindex flag.

7. **AC7 - Configuration listing exposes metadata but not secrets.** `TenantConfigurationView.EmbeddingConfig` serializes the new OIDC metadata fields and `apiSecretKeyName` as a secret-name reference. Tests prove a sample secret value such as `super-secret-client-secret` is not present in the JSON.

8. **AC8 - No duplicate projection is introduced.** Do not create a parallel `TenantEmbeddingConfigView` or manually copy fields unless a committed prerequisite makes direct serialization impossible. The existing `TenantConfigurationView` embeds the full `TenantEmbeddingConfig` by design.
...
- [ ] Task 1 - Add actor state migration and round-trip tests (AC: #1-#5)
  ...
  - [ ] Add or update a breaking-change test for Ollama `BaseUrl` to match Story 13.4's `GetBreakingChangeFields(...)` contract.
...
- [ ] Task 2 - Pin embedding-config endpoint serialization (AC: #6, #10)
  ...
  - [ ] Assert the response body exposes the secret name, for example `memories-embedding-client-secret`, and does not expose a sample secret value.
...
- [ ] Task 3 - Pin tenant configuration/listing surface (AC: #7, #8)
  ...
  - [ ] Assert no `client_secret` value or sample raw secret string appears in serialized JSON.
...
- `TryGetStoredEmbeddingConfigAsync()` returns Google defaults on `JsonException` or `ArgumentException`. This is fallback for corrupt or invalid state, not a migration write-back path.
...
- Do not add a `TenantEmbeddingConfigView` unless direct serialization fails for a concrete reason. A projection creates a new place to forget future fields.
...
- Test data may include a sample raw secret string only to assert it is absent from serialized output.
