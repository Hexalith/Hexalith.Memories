...
| 2026-05-02 | D1 resolved by Jerome (`do best` autonomy): defer tolerant-defaults bundle to Story 13.4 as `13.1-RV11`. All ACs pass, no unresolved patches, deferred items tracked. Status: review → done. | claude-opus-4-7 |
...
- [x] **[Review][Decision→Defer] D1 → 13.1-RV11 — Tolerant defaults pattern in `Validate(...)`.** Resolved 2026-05-02 by Jerome (`do best` autonomy). Decision: defer to Story 13.4 as `13.1-RV11`. Rationale: (1) AC11 hard-bounds 13.1 to 2 files — extending now violates file-scope discipline; (2) Story 13.4 will gain OIDC fields on `TenantEmbeddingConfig`, requiring a fresh `Validate(...)` re-pass that is the natural home for provider→{model-allowlist}→{dim-allowlist} coupling and the regex tightening; (3) mirrors the Round 2 elicitation rollback pattern that moved `ParseProvider` to Story 13.3. Tracked at `13.1-RV11` in deferred-work.md.

#### Deferred (tracked debt — added to `_bmad-output/implementation-artifacts/deferred-work.md`)

- [x] **[Review][Defer] 13.1-RV11 — Tolerant defaults in `Validate(...)`: cross-pollinated configs slip through (HIGH).** The dim assertion is keyed on model name only (not provider+model), and `ModelNamePattern` (`^[A-Za-z0-9.:_-]+$`) requires no alphanumeric. Validator currently accepts: (a) `Provider="google", Model="qwen3-embedding:4b", Dimensions=2560`; (b) `Provider="ollama", Model="gemini-embedding-001", Dimensions=768`; (c) `Provider="ollama", Model="totally-fake", Dimensions=1`; (d) `Model=":::"` / `Model="-"`. **Owner:** Story 13.4. **Action:** when 13.4 introduces OIDC fields and re-touches Validate, also (1) introduce a `provider→{model→dim-allowlist}` registry, (2) tighten regex to `^[A-Za-z0-9][A-Za-z0-9.:_-]*$`, (3) add cross-pollination negative tests. **Re-open trigger if 13.4 ships without it:** any downstream Story 13.2/13.3 test that has to special-case a cross-provider config; or any operational incident where a tenant config validates clean but errors at embedding time. Sources: Blind B2/B4 + Edge E1/E2/E3/E7/E8 (bundled per `feedback_tolerance_idioms.md`). (`src/Hexalith.Memories.Server/Ingestion/EmbeddingProviderDefaults.cs:117-145, 167`)
