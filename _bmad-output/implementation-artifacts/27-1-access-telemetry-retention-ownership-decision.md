...
- [ ] [Review][Patch] [High] Replace Kubernetes clock Jobs/Leases and Pod identity with a platform-neutral clock-attestation service invoked through Dapr service invocation; bind short-lived attestations to deployment, service-instance, process, and Redis-topology identities, with Kubernetes retained only as an optional deployment adapter [`docs/dev/adr-27.1-001-access-telemetry-lifecycle.md:230-270`]
