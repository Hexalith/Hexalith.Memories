...
- Access telemetry lifecycle — current behavior is JSON-console emission plus optional OTLP export, with no repository-owned bounded lifecycle. [ADR 27.1-001](../../docs/dev/adr-27.1-001-access-telemetry-lifecycle.md) accepts a separate Redis 7.4 write-only access-telemetry workload: two StatefulSet data members on retained encrypted PVCs; three independently spread Sentinel members; TLS-only, default-deny network access; typed-state sanitization; continuously signed independent-UTC attestations; millisecond Redis-clock expiry; fenced active/passive lifecycle control; synchronous cohort-specific key/index reclamation; controller-triggered persisted-file compaction; provider-specific filtering and non-blocking buffering; separate writer/lifecycle/compaction/inspection authorities; and explicit durability and rollback boundaries. Its ratified all-nine-operation envelope is 250 events/s cluster-wide with 512 GiB configured Redis memory and a 1.5-TiB PVC per data member, keeping the 7-day maximum at 63.37% memory and 37.55% PVC including rewrite workspace. Stories 27.2 and 27.3 are unblocked to implement and verify the accepted target. `20.5-A41-ACCESS-TELEMETRY-RETENTION` remains open until implementation and Production-shaped evidence pass or an explicit accepted-debt disposition satisfies the recorded closure gate. This is bounded infrastructure telemetry, not tamper-evident, append-only, legally compliant, or certified audit retention.
