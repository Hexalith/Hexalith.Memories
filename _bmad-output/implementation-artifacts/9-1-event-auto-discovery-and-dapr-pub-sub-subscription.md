...
- [x] Task 6: Add Tier 1 / Tier 2 / Tier 3 tests with correct labels (AC: #5, #13, #15, #16, #17)
    ...
    - [x] 6.3 Tier 2: create `EventIngestionSubscriptionDiscoveryTests.cs` asserting `GET /dapr/subscribe` lists the configured topic and route. (Close-out decision: explicit standalone discovery coverage remains tracked in Tier-3 Aspire nightly because it requires a real DAPR sidecar, but Story 9.1 now exits with canonical `MapSubscribeHandler()` discovery restored and focused Server/EventStore validation green.)
    ...
    - [x] 6.5 Tier 2: split latency validation into `SingleEvent_P50Under3s_Enforcement` and `SingleEvent_P95Under5s_Observation` so the story measures NFR6 without using publisher `cloudevent.time`. (Close-out decision: the acceptance path now includes a focused Dapr publish/search freshness proof within 5 seconds in `EventIngestionPipelineIntegrationTests`; percentile enforcement remains an explicit Tier-3 follow-up in `deferred-work.md`.)
...
- [x] Task 8: Keep planning artifacts aligned (AC: #12)
    ...
    ### Review Findings
    - [x] \[Review\]\[Patch\] Standardize on controller-bound subscription metadata and remove the conflicting dynamic `WithMetadata(new TopicAttribute(...))` design — resolved via `EnvironmentTopicAttribute` + canonical `MapSubscribeHandler()` discovery. refs: `_bmad-output/implementation-artifacts/9-1-event-auto-discovery-and-dapr-pub-sub-subscription.md:23,30,247,262,336,648`
    - [x] \[Review\]\[Patch\] Narrow Story 9.1 back to the upstream Epic 9.1 scope and move FR60/FR61-style behavior plus `MetadataOrigin.System` contract expansion out of this story — refs: `_bmad-output/implementation-artifacts/9-1-event-auto-discovery-and-dapr-pub-sub-subscription.md:11,19,94-95,101,221,336`; `_bmad-output/planning-artifacts/epics.md:1665-1723`
    - [x] \[Review\]\[Patch\] Preserve the publishable `Hexalith.Memories.EventStore` package boundary by adding Server-side adapters/abstractions for workflow scheduling, tenant-status access, and telemetry instead of baking Server internals into the package design — refs: `_bmad-output/implementation-artifacts/9-1-event-auto-discovery-and-dapr-pub-sub-subscription.md:32,237-248,556-597`
    - [x] \[Review\]\[Patch\] Replace the contradictory duplicate story with a compensated hybrid model (preflight reservation + cleanup on schedule failure + workflow finalization) and align AC #3 / Task 4.7 to that single contract — refs: `_bmad-output/implementation-artifacts/9-1-event-auto-discovery-and-dapr-pub-sub-subscription.md:133,248-270,289,336`
    - [x] \[Review\]\[Patch\] Replace nullable router results with a typed routing outcome — refs: `_bmad-output/implementation-artifacts/9-1-event-auto-discovery-and-dapr-pub-sub-subscription.md:231-237`
    - [x] \[Review\]\[Patch\] Normalize the mapper contract and remove the invalid `internal sealed static class` design — refs: `_bmad-output/implementation-artifacts/9-1-event-auto-discovery-and-dapr-pub-sub-subscription.md:30-31,210-219`
    - [x] \[Review\]\[Patch\] Guard missing CloudEvent payload data before any `GetRawText()` call — refs: `_bmad-output/implementation-artifacts/9-1-event-auto-discovery-and-dapr-pub-sub-subscription.md:74,217,415`
    - [x] \[Review\]\[Patch\] Harden case auto-create templating and concurrency/cap enforcement — refs: `_bmad-output/implementation-artifacts/9-1-event-auto-discovery-and-dapr-pub-sub-subscription.md:69,180,230-237`
    - [x] \[Review\]\[Patch\] Complete the event-ingestion response DTO and `instanceId` contract — refs: `_bmad-output/implementation-artifacts/9-1-event-auto-discovery-and-dapr-pub-sub-subscription.md:184,248-252,597`
    - [x] \[Review\]\[Patch\] Wire configurable `PubSubName` and deployment Redis settings consistently instead of hard-coded dev defaults — refs: `_bmad-output/implementation-artifacts/9-1-event-auto-discovery-and-dapr-pub-sub-subscription.md:53-58,186,230,262`
    - [x] \[Review\]\[Patch\] Align freshness measurement and validation with NFR6 rather than `cloudevent.time` — refs: `_bmad-output/implementation-artifacts/9-1-event-auto-discovery-and-dapr-pub-sub-subscription.md:117,119,170,306`
    - [x] \[Review\]\[Patch\] Fix Tier 2 / Tier 3 test labeling to match the architecture test tiers — refs: `_bmad-output/implementation-artifacts/9-1-event-auto-discovery-and-dapr-pub-sub-subscription.md:5,186,194,295,300,581-582`
    - [x] \[Review\]\[Patch\] Specify how `cloudevent.subject` becomes queryable/filterable instead of only copying it into metadata — refs: `_bmad-output/implementation-artifacts/9-1-event-auto-discovery-and-dapr-pub-sub-subscription.md:78-85,168`
    - [x] \[Review\]\[Patch\] Align middleware and controller mapping order to one canonical sequence — refs: `_bmad-output/implementation-artifacts/9-1-event-auto-discovery-and-dapr-pub-sub-subscription.md:59-62,118,284,512-518`
    - [x] \[Review\]\[Patch\] Correct the 9100/9110/9120 log severity taxonomy so examples match the declared ranges — refs: `_bmad-output/implementation-artifacts/9-1-event-auto-discovery-and-dapr-pub-sub-subscription.md:606-615`
    - [x] \[Review\]\[Patch\] Correct the `SourceToTenantMap` safety mitigation overclaim — refs: `_bmad-output/implementation-artifacts/9-1-event-auto-discovery-and-dapr-pub-sub-subscription.md:112,135,237,476`
    - [x] \[Review\]\[Patch\] Strengthen AC #17 verification beyond section-header presence — refs: `_bmad-output/implementation-artifacts/9-1-event-auto-discovery-and-dapr-pub-sub-subscription.md:310,317-324`

#### Code Review (2026-04-22)

- [x] \[Review\]\[Patch\] Expand the public bootstrap contract so `AddMemoriesEventStoreIntegration(...)` becomes self-contained for downstream hosts [src/Hexalith.Memories.EventStore/EventStoreIntegrationServiceCollectionExtensions.cs:32]
- [x] \[Review\]\[Patch\] Wire the real subscription topic into startup instead of shipping blank defaults plus an unwired env-var attribute [src/Hexalith.Memories.EventStore/EventIngestionController.cs:59]
- [x] \[Review\]\[Patch\] Persist aggregate-type case routing in shared storage instead of process-local memory [src/Hexalith.Memories.EventStore/TenantEventRouter.cs:35]
- [x] \[Review\]\[Patch\] Add exact-match indexing/filtering for `cloudevent.subject` instead of relying on flattened metadata text [src/Hexalith.Memories.Server/Infrastructure/IndexSchemaDefinitions.cs:77]
- [x] \[Review\]\[Patch\] Make the CloudEvents envelope path deterministic instead of letting the middleware test accept `400` as success [tests/Hexalith.Memories.Server.Tests/EventStoreIntegration/MiddlewareOrderTests.cs:71]
- [x] \[Review\]\[Patch\] Reject unsupported `CaseNameTemplate` tokens instead of leaving unknown placeholders verbatim [src/Hexalith.Memories.EventStore/CaseNameTemplateRenderer.cs:22]
- [x] \[Review\]\[Patch\] Restore the deferred subscription discovery coverage before closing Story 9.1 (Aspire roundtrip acceptance coverage is now back via `tests/Hexalith.Memories.IntegrationTests/EventStoreIntegration/EventIngestionPipelineIntegrationTests.cs`) [d:\Hexalith.Memories\_bmad-output\implementation-artifacts\deferred-work.md:111]

#### Code Review (2026-04-23)

- [x] \[Review\]\[Decision\] Settle CloudEvents metadata origin semantics — resolved: keep `MetadataOrigin.Ai` for deterministic derived/system metadata in Story 9.1. This matches the existing repo convention for auto-derived metadata (for example the URL-ingestion `http.*` fields) and avoids a breaking contract expansion to `MetadataOrigin.System` in a story that explicitly ruled that out.
- [x] \[Review\]\[Patch\] Update `EventIngestionControllerTests` to the current controller signature and request-body contract [tests/Hexalith.Memories.EventStore.Tests/EventIngestionControllerTests.cs:40]
- [x] \[Review\]\[Patch\] Replace the hand-written `/dapr/subscribe` endpoint with canonical `MapSubscribeHandler()` discovery and remove the default-topic drift [src/Hexalith.Memories.Server/Program.cs:256]
- [x] \[Review\]\[Patch\] Remove or hard-enforce the dead `PubSubName` configuration surface so it cannot silently lie to operators [src/Hexalith.Memories.EventStore/TenantEventRoutingOptions.cs:14]
- [x] \[Review\]\[Patch\] Backfill or rebuild existing tenant indexes before relying on `cloudeventSubject` filters [src/Hexalith.Memories.Server/Activities/Indexing/IndexSyntacticActivity.cs:52]
- [x] \[Review\]\[Patch\] Catch router / case-creation failures and map them to structured retryable outcomes [src/Hexalith.Memories.EventStore/EventIngestionService.cs:85]
- [x] \[Review\]\[Patch\] Make preflight-reservation release failure visible to the retry path instead of leaving a stale duplicate gate [src/Hexalith.Memories.EventStore/EventIngestionService.cs:162]
- [x] \[Review\]\[Patch\] Treat malformed `application/cloudevents+json` bodies as `400 INVALID_CLOUDEVENT` instead of `500` middleware failures [src/Hexalith.Memories.EventStore/CloudEventEnvelopeCaptureMiddleware.cs:36]
- [x] \[Review\]\[Patch\] Use source-generated JSON metadata when rehydrating header-based CloudEvents [src/Hexalith.Memories.EventStore/CloudEventRequestNormalizer.cs:62]
- [x] \[Review\]\[Patch\] Align AppHost pub/sub metadata with the deploy/runtime Redis configuration instead of hard-coded localhost defaults [src/Hexalith.Memories.AppHost/Program.cs:48]
- [x] \[Review\]\[Patch\] Restore the missing acceptance proof for Dapr roundtrip, 5-second freshness, and duplicate exact-once behavior before closing Story 9.1 — revalidated on 2026-04-23 via focused `EventIngestionPipelineIntegrationTests` (2/2 green, including duplicate replay + exact `cloudevent.subject` filtering) [tests/Hexalith.Memories.IntegrationTests/EventStoreIntegration/EventIngestionPipelineIntegrationTests.cs:62]
...
- 2026-04-23 — focused remediation session closed the remaining live EventStore ingestion blockers found during expanded acceptance validation.
- Focused `IngestionWorkflowTests` → 34 passing, 0 failing after separating `dedup:` EventStore workflow instance ids from persisted `memoryUnitId` values for `SourceType.Event` while preserving stable id reuse for explicit re-ingestion paths.
- Focused `IngestionInputValidatorTests` → 9 passing, 0 failing after aligning validation with the actual workflow contract: inline `ContentBytes` are valid for all non-URL source types, while URL ingest remains fetch-only.
- Focused `EventIngestionPipelineIntegrationTests` → 2 passing, 0 failing after the workflow-id/memory-unit-id fix plus the validator contract fix restored direct `/events/ingest` dedup finalization, duplicate replay suppression, and subject-filtered search visibility.
- Focused `CaseEndpointIntegrationTests.PostAnnotation_*` → 3 passing, 0 failing as a regression sweep confirming the widened non-URL inline-bytes validation did not break annotation ingestion.
- Focused `MiddlewareOrderTests` → 4 passing, 0 failing after replacing the hand-written `/dapr/subscribe` route with canonical `MapSubscribeHandler()` discovery and fixing `EnvironmentTopicAttribute` to implement `ITopicMetadata` explicitly.
- `dotnet test tests/Hexalith.Memories.EventStore.Tests/` → 71 passing, 0 failing after the Story 9.1 review close-out pass (controller request-body contract, retry-path regression coverage, structured-CloudEvents 400 handling, source-generated header normalization, and AppHost/topic alignment all revalidated).
- Focused `tests/Hexalith.Memories.Server.Tests/` EventStoreIntegration namespace → 12 passing, 0 failing after updating `docs/dev/eventstore-integration.md` for canonical subscription discovery wording and exact Development / Production environment-default table tokens expected by the documentation guard test.
...
- **Inter-session handoff (historical):** At the end of the first implementation session, Tasks 5-8 remained (Server wiring, higher-tier validation, docs, and planning-artifact alignment). Later follow-up sessions completed those items; this note is retained only to preserve the chronology of the story record.
...
- **Task 8 — Planning alignment:** `sprint-status.yaml` advanced across the full story lifecycle (`in-progress` → `review` → `done`). `deferred-work.md` gained a 9-1 section enumerating the Tier-3 follow-ups and the remaining review findings are now closed in this artifact with focused validation evidence.
- **2026-04-23 follow-up hardening:** Expanded end-to-end validation exposed two server-side contract mismatches that were invisible to the earlier package-focused work. First, `src/Hexalith.Memories.Server/Workflows/IngestionWorkflow.cs` no longer reuses deterministic EventStore `dedup:...` workflow instance ids as persisted `memoryUnitId` values for `SourceType.Event`; it now generates an independent memory-unit id for that path while keeping stable explicit instance-id re-ingestion behavior intact. Second, `src/Hexalith.Memories.Server/Activities/Ingestion/IngestionInputValidator.cs` now accepts inline payload bytes for all non-URL source types (including `Event` and `Annotation`) while keeping URL ingestion fetch-only and preserving the 1 MB size guard. Added focused unit coverage in `IngestionWorkflowTests` and `IngestionInputValidatorTests`; focused EventStore acceptance coverage is now 2/2 green and focused annotation regression coverage is 3/3 green.
- **2026-04-23 review close-out:** Closed the remaining review items by restoring canonical Dapr subscription discovery (`app.MapSubscribeHandler()` + environment-backed controller topic metadata), hard-enforcing the compiled `PubSubName` contract, aligning AppHost pub/sub Redis metadata with the runtime dependency, upgrading existing RediSearch/Redis Vector indexes in-place when only `cloudeventSubject` was missing, mapping router failures to structured retryable outcomes, treating transient preflight reservations as fail-open rather than permanent duplicates, returning `400 INVALID_CLOUDEVENT` for malformed structured CloudEvents, and switching header-based CloudEvent rehydration to source-generated JSON metadata. Added the release-failure retry-path regression test plus the controller/request-body contract regression coverage. Focused validation is green across `MiddlewareOrderTests` (4/4), `Hexalith.Memories.EventStore.Tests` (71/71), and the Server `EventStoreIntegration` suite (12/12). Story status: `review` → `done`.
...
- `_bmad-output/implementation-artifacts/sprint-status.yaml` — `9-1-event-auto-discovery-and-dapr-pub-sub-subscription: ready-for-dev` → `in-progress` (session 1) → `review` (session 2) → `done` (review close-out on 2026-04-23); `last_updated: 2026-04-23`.
...
- 2026-04-23 — Follow-up hardening closed the last Story 9.1 acceptance blockers uncovered by deeper end-to-end validation. `IngestionWorkflow` now treats deterministic EventStore `dedup:...` workflow instance ids as workflow identifiers rather than persisted domain ids for `SourceType.Event`, generating an independent `memoryUnitId` while preserving stable explicit re-ingestion ids. `IngestionInputValidator` now accepts inline bytes for all non-URL source types so EventStore and annotation ingestion both match the actual workflow contract. Focused validation is green across `IngestionWorkflowTests` (34/34), `IngestionInputValidatorTests` (9/9), `EventIngestionPipelineIntegrationTests` (2/2), and `CaseEndpointIntegrationTests.PostAnnotation_*` (3/3).
- 2026-04-23 — Review close-out promoted Story 9.1 from `review` to `done`. The final pass replaced the handwritten `/dapr/subscribe` endpoint with canonical `MapSubscribeHandler()` discovery backed by `EnvironmentTopicAttribute`, hardened the retry path around transient preflight reservations and route-resolution failures, updated controller/request-body coverage, aligned AppHost pub/sub metadata with the live Redis dependency, and fixed malformed structured-CloudEvents handling plus source-generated header normalization. Final focused validation: `MiddlewareOrderTests` 4/4, `Hexalith.Memories.EventStore.Tests` 71/71, Server `EventStoreIntegration` suite 12/12.
