...
**Qualification and close-out split (corrected 2026-08-01 by approved Sprint Change Proposal 2026-08-01):** Story 27.3 owns C0 and the independent C2/C3/C4 adapter qualification against the immutable `PG-ONPREM-1` definition. All twenty-five running-target C1 gates are held without a registered story owner. Story 27.4 owns deployment-shaped lifecycle evidence, operations documentation, and A41 close-out, but cannot start while either successor file/registration is absent or any C1 gate is held, unproven, stale, or failed. Story 27.3 reaching `done` never enables Production lifecycle writes or advances Story 27.4; A41 remains open and outside Story 27.3's mutation authority.
...
- Actual Story 27.5 and Story 27.6 files exist, satisfy `story-scope-guard.md` and the checkpoint-heavy story guard above, are registered by an approved correction, and are both `done`. Absence of either file or registration fails this predecessor gate.
...
- Stories 27.3, 27.5, 27.6, and 27.4 use the same immutable profile hash. Any mismatch keeps Production lifecycle writes disabled, Story 27.4 `backlog`, and A41 open.
...
**Re-registration gate:** a later approved correction may register successor stories only after the separately tracked story files exist, every retained checkpoint names a real re-runnable producer command or evidence artifact, each story carries a current Historical Context Classification and slice proof, and `tools/check-story-slice-scope.py --require-record` evaluates those files. Until then no story owns C1.1-C1.25, Production lifecycle writes remain disabled, Story 27.4 remains `backlog`, and A41 remains open.
