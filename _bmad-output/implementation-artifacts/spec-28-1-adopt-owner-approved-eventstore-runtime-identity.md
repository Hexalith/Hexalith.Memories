...
- [~] `references/Hexalith.Builds` gitlink and `Directory.Packages.props` chain -- Ask-First #1 resolved by the human: added commit `069d1168d88b53a4854bed1f6f52685b2283c6e2` on top of prior HEAD `b6a0d1bd2c4b4398f0a6b1be15d0f3b698f250a5` overriding only `HexalithEventStoreVersion` to `999.1.20-proof.fa2d1c9910f8` in `Props/Directory.Packages.props` (commitlint-validated `fix(deps): pin approved EventStore proof version for story 28.1`, committed locally, not pushed); Memories' `references/Hexalith.Builds` gitlink bumped to that commit. **Restorability verified NEGATIVE, not fabricated**: (1) `https://api.nuget.org/v3-flatcontainer/hexalith.eventstore.client/999.1.20-proof.fa2d1c9910f8/...nupkg` -> HTTP 404; not in the local NuGet cache; the proof packet's `raw_evidence_bundle_url` (Azure blob) -> HTTP 404 (no SAS/auth available here). (2) Ran the project's own release recipe, `python3 tools/pack-release-packages.py <dir> 999.1.20-proof.fa2d1c9910f8` (from `references/Hexalith.EventStore` at the pinned SHA, invoked with cwd = repo root so the *root* `global.json` (SDK 10.0.400) applies instead of EventStore's own strict `10.0.302`), which produced all 14 real, loadable `.nupkg` files -- but their SHA-256 hashes do **not** match any of the 14 per-package hashes in the proof packet's approved manifest (expected, since AD-11 pins SDK 10.0.302/ASP.NET 10.0.10 and this sandbox only has 10.0.400/10.0.11 -- not a reproducible-build environment). (3) An ephemeral, uncommitted probe `NuGet.config` (local feed + `signatureValidationMode=accept`, scratch-only, never merged into the tracked config) proved these locally-packed bytes are mechanically restorable/buildable, then was discarded -- this is explicitly **not** offered as satisfying the AC, since the bytes are not hash-identical to the approved artifact and the tracked `NuGet.config` was never changed. **New finding beyond the original Ask-First framing**: `src/Hexalith.Memories.Server/Hexalith.Memories.Server.csproj` has an *unconditional* `PackageReference` to `Hexalith.EventStore.Client` (Code Map line 51) with no source-mode `ProjectReference` fallback, so this version bump breaks restore (`NU1102`) for **both** Release/package mode and Debug/source mode (`-p:UseHexalithProjectReferences=true`) for every project that depends on `Hexalith.Memories.Server` (Server, Server.Tests, AppHost, IntegrationTests, MigrateEmbeddingVectors, DerivedStores.Tests, Benchmarks) -- confirmed with a clean NuGet cache; see Verification. Package-identity AC is **not satisfied**; this needs a further human decision (real publish, a sanctioned isolated feed wired into the tracked `NuGet.config`, or a Server.csproj source-mode conditional) before it can be.
- [x] `src/Hexalith.Memories.AppHost/Program.cs` -- added `eventStoreGateway = builder.AddHexalithEventStoreGatewayProject()` (default name `"eventstore"`, never `AddHexalithEventStore(...)`), with a Dapr sidecar (`AppId "eventstore"`, HTTP port 3501, gRPC port 50002 -- both otherwise-unused in this AppHost) referencing the existing `stateStore`/`pubSub` builders (both sidecar-level `WithReference` and project-level `WithReference`, mirroring `memories` at Program.cs:298-301,319-325) -- no new/duplicate Dapr components created. Satisfies single-`eventstore`-resource AC. Verified by isolated build (see Verification) since the package-identity blocker above prevents a full-solution build at the final pinned version.
- [ ] `tests/Hexalith.Memories.IntegrationTests/EventStoreIntegration/` -- **blocked**: cannot write a verifiable test because `Hexalith.Memories.IntegrationTests.csproj` cannot currently restore/build at all (same `NU1102` as above, transitively through `Hexalith.Memories.Server`). Not attempted, to avoid fabricating unverifiable test code.
- [x] CI/build scripts -- added a `Restore solution (Debug/source)` + `Build solution (Debug/source)` step pair to the `build` job in `.github/workflows/ci.yml` (`-p:UseHexalithProjectReferences=true`), alongside the existing Release/package build. Note: this CI step will itself now fail once it runs, for the same package-identity reason above, until that is resolved.
