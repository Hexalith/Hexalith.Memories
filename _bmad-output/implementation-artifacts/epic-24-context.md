...
- Preserve zero cross-tenant leakage across ingestion, every search axis, and graph traversal. Evidence must cover malformed, missing, swapped, and colliding tenant identifiers; graph proof requires identical tenant A/B structures and edge identifiers with distinct payload markers, then authenticated traversal returning no foreign nodes or edges.
