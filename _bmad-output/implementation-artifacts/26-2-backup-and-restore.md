...
**Previous-story continuity (resolved):** Story 26.2 may be planned or re-driven only from a descendant where Story 26.1 is `done` and its mandatory zero-skip disposable-cluster rollout evidence is recorded. The current repository state satisfies that prerequisite; a re-drive must consume the completed Story 26.1 deployment artifacts and must not reopen or weaken its rollout gate.
