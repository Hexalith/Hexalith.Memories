...
- `.github/workflows/commitlint.yml` -- pass the exact PR title required by the reusable gate and align CI validation with the repository-local configuration.
...
- `.githooks/commit-msg` and `commitlint.config.mjs` -- repository-local Conventional Commit enforcement.
- `.github/workflows/commitlint.yml` -- root caller for mandatory pull-request title validation and CI commitlint alignment.
